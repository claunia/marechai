// software-cover-upload.js — XHR-based Software cover batch upload helper for the
...
//   * Used by: Marechai/Pages/Suggestions/SoftwareCoversSuggestionDialog.razor

window.MarechaiSoftwareCoverUpload = (function () {
     ...
     */
    function uploadSoftwareCovers(apiBaseUrl, jwtToken, releaseId, fileInputElementOrId, dotNetRef) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/software/covers/pending?releaseId=${encodeURIComponent(releaseId)}`;
    ...
    /**
     * Fetch a pending Software cover image as a blob URL and assign it to the given <img>'s src.
     * Mirrors MarechaiPendingCover.applyPendingCoverImage but for the software-covers item folder.
     */
    async function applyPendingPhotoImage(apiBaseUrl, jwtToken, guid, imgElementOrId) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/software/covers/pending/${encodeURIComponent(guid)}`;
    ...
    /**
     * Open a pending Software cover image in a new tab via a blob URL (admin lightbox).
     */
