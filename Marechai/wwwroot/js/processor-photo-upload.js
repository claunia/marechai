// processor-photo-upload.js — XHR-based processor photo batch upload helper for the
// collaborative-suggestion dialog.
//
// Mirrors gpu-photo-upload.js verbatim with entity-name swap (GPU → processor).
// See gpu-photo-upload.js header for design notes.
//
// Used by: Marechai/Pages/Suggestions/ProcessorPhotosSuggestionDialog.razor

window.MarechaiProcessorPhotoUpload = (function () {
     ...
     */
    function uploadProcessorPhotos(apiBaseUrl, jwtToken, processorId, fileInputElementOrId, dotNetRef) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/processors/photos/pending?processorId=${encodeURIComponent(processorId)}`;
    ...
    /**
     * Fetch a pending processor photo as a blob URL and assign it to the given <img>'s src.
     * Mirrors MarechaiPendingCover.applyPendingCoverImage but for the processors item folder.
     */
    async function applyPendingPhotoImage(apiBaseUrl, jwtToken, guid, imgElementOrId) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/processors/photos/pending/${encodeURIComponent(guid)}`;
    ...
    /**
     * Open a pending processor photo in a new tab via a blob URL (admin lightbox).
     */
