// software-cover-batch-upload.js — XHR-based admin batch upload helper for the
// SoftwareCovers admin page.
//
// Differs from software-cover-upload.js (suggestions):
//   * Hits /software/covers/admin/pending (Admin/UberAdmin only, 7-format whitelist).
...
window.MarechaiSoftwareCoverBatchUpload = (function () {
     ...
     *   OnCoverUploadStarted(clientGuid, fileName, sizeBytes)
     *   OnCoverUploadProgress(clientGuid, percent)
     *   OnCoverUploadCompleted(clientGuid, serverGuid, extension, thumbnailBase64, sizeBytes, width, height)
     *   OnCoverUploadFailed(clientGuid, errorMessage)
     *
     * @param {string} apiBaseUrl Absolute base URL of the API server.
     * @param {string} jwtToken   JWT bearer token (no "Bearer " prefix).
     * @param {number} releaseId  Parent SoftwareRelease id (sent as ?releaseId= query string).
     ...
     */
    function uploadBatch(apiBaseUrl, jwtToken, releaseId, fileInputElementOrId, dotNetRef, remainingSlots) {
        ...
        for (const file of Array.from(input.files)) {
            if (slotsLeft <= 0) {
                rejected++;
                try { dotNetRef.invokeMethodAsync('OnCoverUploadFailed', cryptoRandomGuid(), `${file.name}: batch cap reached (25)`); }
                catch { /* ignored */ }
                continue;
            }
            if (file.size > 50 * 1024 * 1024) {
                rejected++;
                try { dotNetRef.invokeMethodAsync('OnCoverUploadFailed', cryptoRandomGuid(), `${file.name}: exceeds 50 MB limit`); }
                catch { /* ignored */ }
                continue;
            }
            ...
            try { dotNetRef.invokeMethodAsync('OnCoverUploadStarted', clientGuid, file.name, file.size); }
            catch { /* ignored */ }
            ...
            xhr.upload.onprogress = function (ev) {
                ...
                try { dotNetRef.invokeMethodAsync('OnCoverUploadProgress', clientGuid, pct); }
                catch { /* ignored */ }
            };
            ...
            xhr.onload = function () {
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    if (body && body.id && body.extension && body.thumbnail_base64) {
                        try {
                            dotNetRef.invokeMethodAsync('OnCoverUploadCompleted',
                                clientGuid, body.id, body.extension, body.thumbnail_base64,
                                body.size_bytes || file.size, body.width || 0, body.height || 0);
                        } catch { /* ignored */ }
                    } else {
                        try { dotNetRef.invokeMethodAsync('OnCoverUploadFailed', clientGuid, `${file.name}: malformed server response`); }
                        catch { /* ignored */ }
                    }
                } else {
                    let detail = '';
                    ...
                    try { dotNetRef.invokeMethodAsync('OnCoverUploadFailed', clientGuid, `${file.name}: ${detail}`); }
                    catch { /* ignored */ }
                }
            };
            ...
            xhr.onerror = function () {
                inFlight.delete(clientGuid);
                try { dotNetRef.invokeMethodAsync('OnCoverUploadFailed', clientGuid, `${file.name}: network error`); }
                catch { /* ignored */ }
            };
