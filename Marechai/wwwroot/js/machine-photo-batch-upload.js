// machine-photo-batch-upload.js — XHR-based admin batch upload helper for the
// Machine Photos admin page.
//
// Differs from machine-photo-upload.js (suggestions) / single-upload:
//   * Hits /machines/photos/admin/pending (Admin/UberAdmin only, 7-format whitelist).
...
window.MarechaiMachinePhotoBatchUpload = (function () {
     ...
     */
    function uploadBatch(apiBaseUrl, jwtToken, machineId, fileInputElementOrId, dotNetRef, remainingSlots) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/machines/photos/admin/pending?machineId=${encodeURIComponent(machineId)}`;
        ...
        for (const file of Array.from(input.files)) {
            ...
            if (!extOk && !mimeOk) {
                rejected++;
                try { dotNetRef.invokeMethodAsync('OnPhotoUploadFailed', cryptoRandomGuid(), `${file.name}: unsupported type (allowed: JPG, PNG, WebP, AVIF, BMP, TIFF)`); }
                catch { /* ignored */ }
                continue;
            }
