// machine-photo-upload.js — XHR-based machine photo batch upload helper for the
// collaborative-suggestion dialog.
//
// Mirrors gpu-photo-upload.js verbatim with entity-name swap (GPU → machine).
// See gpu-photo-upload.js header for design notes.
//
// Used by: Marechai/Pages/Suggestions/MachinePhotosSuggestionDialog.razor

window.MarechaiMachinePhotoUpload = (function () {
     ...
     * @param {number} machineId Parent machine id (sent as ?machineId= query string).
     ...
     */
    function uploadMachinePhotos(apiBaseUrl, jwtToken, machineId, fileInputElementOrId, dotNetRef) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/machines/photos/pending?machineId=${encodeURIComponent(machineId)}`;
    ...
    /**
     * Fetch a pending machine photo as a blob URL and assign it to the given <img>'s src.
     * Mirrors MarechaiPendingCover.applyPendingCoverImage but for the machines item folder.
     */
    async function applyPendingPhotoImage(apiBaseUrl, jwtToken, guid, imgElementOrId) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/machines/photos/pending/${encodeURIComponent(guid)}`;
    ...
    /**
     * Open a pending machine photo in a new tab via a blob URL (admin lightbox).
     */
