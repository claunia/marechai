// software-screenshot-upload.js — XHR-based Software screenshot batch upload helper for the
...
//   * Used by: Marechai/Pages/Suggestions/SoftwareScreenshotsSuggestionDialog.razor

window.MarechaiSoftwareScreenshotUpload = (function () {
     ...
     */
    function uploadSoftwareScreenshots(apiBaseUrl, jwtToken, softwareId, fileInputElementOrId, dotNetRef) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/software/screenshots/pending?softwareId=${encodeURIComponent(softwareId)}`;
    ...
    /**
     * Fetch a pending Software screenshot image as a blob URL and assign it to the given <img>'s src.
     * Mirrors MarechaiPendingCover.applyPendingCoverImage but for the software-screenshot item folder.
     */
    async function applyPendingPhotoImage(apiBaseUrl, jwtToken, guid, imgElementOrId) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/software/screenshots/pending/${encodeURIComponent(guid)}`;
    ...
    /**
     * Open a pending Software screenshot image in a new tab via a blob URL (admin lightbox).
     */
