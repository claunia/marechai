// sound-synth-photo-batch-upload.js — XHR-based admin batch upload helper for
// the Sound Synth Photos admin page.
//
// Differs from sound-synth-photo-upload.js (suggestions) / single-upload:
//   * Hits /sound-synths/photos/admin/pending (Admin/UberAdmin only, 7-format whitelist).
...
window.MarechaiSoundSynthPhotoBatchUpload = (function () {
     ...
     */
    function uploadBatch(apiBaseUrl, jwtToken, soundSynthId, fileInputElementOrId, dotNetRef, remainingSlots) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/sound-synths/photos/admin/pending?soundSynthId=${encodeURIComponent(soundSynthId)}`;
