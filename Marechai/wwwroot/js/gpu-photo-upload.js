// gpu-photo-upload.js — XHR-based GPU photo batch upload helper for the
...
//   * Used by: Marechai/Pages/Suggestions/GpuPhotosSuggestionDialog.razor

window.MarechaiGpuPhotoUpload = (function () {
     ...
     * @param {number} gpuId      Parent GPU id (sent as ?gpuId= query string).
     ...
     */
    function uploadGpuPhotos(apiBaseUrl, jwtToken, gpuId, fileInputElementOrId, dotNetRef) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/gpus/photos/pending?gpuId=${encodeURIComponent(gpuId)}`;
    ...
    /**
     * Fetch a pending GPU photo as a blob URL and assign it to the given <img>'s src.
     * Mirrors MarechaiPendingCover.applyPendingCoverImage but for the gpus item folder.
     */
    async function applyPendingPhotoImage(apiBaseUrl, jwtToken, guid, imgElementOrId) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/gpus/photos/pending/${encodeURIComponent(guid)}`;
    ...
    /**
     * Open a pending GPU photo in a new tab via a blob URL (admin lightbox).
     */
