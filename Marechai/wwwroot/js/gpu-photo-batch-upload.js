// gpu-photo-batch-upload.js — XHR-based admin batch upload helper for the
// GPU Photos admin page.
//
// Differs from gpu-photo-upload.js (suggestions) / single-upload:
//   * Hits /gpus/photos/admin/pending (Admin/UberAdmin only, 7-format whitelist).
...
window.MarechaiGpuPhotoBatchUpload = (function () {
     ...
     */
    function uploadBatch(apiBaseUrl, jwtToken, gpuId, fileInputElementOrId, dotNetRef, remainingSlots) {
        ...
        const url = `${apiBaseUrl.replace(/\/+$/, '')}/gpus/photos/admin/pending?gpuId=${encodeURIComponent(gpuId)}`;
