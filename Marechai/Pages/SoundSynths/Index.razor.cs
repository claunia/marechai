...
public partial class Index : IAsyncDisposable
{
    ...
    readonly string              _sentinelId         = $"soundsynths-sentinel-{Guid.NewGuid():N}";
    readonly string              _scrollerId         = $"soundsynths-scroll-{Guid.NewGuid():N}";
    readonly List<SoundSynthDto> _synths             = [];
    bool                         _initialized;
    bool                         _loadingMore;
    ...
    bool HasMore => _synths.Count < _total;
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if(!_initialized)
        {
            ...
            Task<List<SoundSynthDto>> firstPageTask = Service.GetAllAsync(0, _pageSize);

            await Task.WhenAll(countTask, firstPageTask);

            _total = countTask.Result;
            _synths.AddRange(firstPageTask.Result);
            _initialized = true;
    ...
    public async Task OnSentinelVisibleAsync()
    {
        ...
        try
        {
            List<SoundSynthDto> next = await Service.GetAllAsync(_synths.Count, _pageSize);
            _synths.AddRange(next);
