...
public partial class MachinePhotosSuggestionDialog : ComponentBase, IAsyncDisposable
{
    ...
    readonly string _fileInputId = $"machine-photo-input-{Guid.NewGuid():N}";

    DotNetObjectReference<MachinePhotosSuggestionDialog> _dotNetRef;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            List<LicenseDto> all = await MachinePhotosService.GetAllLicensesAsync();
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        foreach(StagedPhoto p in _staged)
        {
            ...
            try
            {
                ...
                await JS.InvokeAsync<object>("MarechaiMachinePhotoUpload.applyPendingPhotoImage",
    ...
    async Task OnFilesPicked()
    {
        try
        {
            ...
            await JS.InvokeAsync<object>("MarechaiMachinePhotoUpload.uploadMachinePhotos",
    ...
    async Task RemoveStagedPhotoAsync(string clientGuid)
    {
        ...
        {
            try { await JS.InvokeVoidAsync("MarechaiMachinePhotoUpload.cancelUpload", p.ClientGuid); } catch { /* ignored */ }
        }

        // Delete the server-side pending file (no-op if upload never completed).
        if(p.ServerGuid != Guid.Empty)
        {
            await MachinePhotosService.DeletePendingPhotoAsync(p.ServerGuid);
        }
    ...
    public Task OnPhotoUploadStarted(string clientGuid, string fileName, long sizeBytes)
    {
        _staged.Add(new StagedPhoto
        {
            ...
            UploadPercent = 0,
            ImgElementId  = $"machine-photo-thumb-{Guid.NewGuid():N}"
        });
        StateHasChanged();
    ...
    async Task SubmitAsync()
    {
        ...
        try
        {
            ...
            {
                EntityType          = (int?)SuggestionEntityType.MachinePhoto,
                EntityId            = MachineId,
    ...
    async Task OnCancelAsync()
    {
        ...
        foreach(StagedPhoto p in _staged.ToList())
        {
            ...
            if(p.ServerGuid != Guid.Empty)
            {
                try { await MachinePhotosService.DeletePendingPhotoAsync(p.ServerGuid); } catch { /* ignored */ }
            }
        }
