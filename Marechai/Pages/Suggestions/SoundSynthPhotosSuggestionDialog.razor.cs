...
public partial class SoundSynthPhotosSuggestionDialog : ComponentBase, IAsyncDisposable
{
    ...
    readonly string _fileInputId = $"sound-synth-photo-input-{Guid.NewGuid():N}";

    DotNetObjectReference<SoundSynthPhotosSuggestionDialog> _dotNetRef;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            List<LicenseDto> all = await SoundSynthPhotosService.GetAllLicensesAsync();
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        foreach(StagedPhoto p in _staged)
        {
            ...
            try
            {
                ...
                await JS.InvokeAsync<object>("MarechaiSoundSynthPhotoUpload.applyPendingPhotoImage",
    ...
    async Task OnFilesPicked()
    {
        try
        {
            ...
            await JS.InvokeAsync<object>("MarechaiSoundSynthPhotoUpload.uploadSoundSynthPhotos",
                                          ApiAssetUrls.BaseUrl, token, SoundSynthId, _fileInputId, _dotNetRef);
        }
        catch(Exception ex)
        {
    ...
    async Task RemoveStagedPhotoAsync(string clientGuid)
    {
        ...
        {
            try { await JS.InvokeVoidAsync("MarechaiSoundSynthPhotoUpload.cancelUpload", p.ClientGuid); } catch { /* ignored */ }
        }

        // Delete the server-side pending file (no-op if upload never completed).
        if(p.ServerGuid != Guid.Empty)
        {
            await SoundSynthPhotosService.DeletePendingPhotoAsync(p.ServerGuid);
        }
    ...
    public Task OnPhotoUploadStarted(string clientGuid, string fileName, long sizeBytes)
    {
        _staged.Add(new StagedPhoto
        {
            ...
            UploadPercent = 0,
            ImgElementId  = $"sound-synth-photo-thumb-{Guid.NewGuid():N}"
        });
        StateHasChanged();
    ...
    async Task SubmitAsync()
    {
        ...
        try
        {
            ...
            {
                EntityType          = (int?)SuggestionEntityType.SoundSynthPhoto,
    ...
    async Task OnCancelAsync()
    {
        ...
        foreach(StagedPhoto p in _staged.ToList())
        {
            ...
            if(p.ServerGuid != Guid.Empty)
            {
                try { await SoundSynthPhotosService.DeletePendingPhotoAsync(p.ServerGuid); } catch { /* ignored */ }
            }
        }
