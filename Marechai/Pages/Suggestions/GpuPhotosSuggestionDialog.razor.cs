...
public partial class GpuPhotosSuggestionDialog : ComponentBase, IAsyncDisposable
{
    ...
    readonly string _fileInputId = $"gpu-photo-input-{Guid.NewGuid():N}";

    DotNetObjectReference<GpuPhotosSuggestionDialog> _dotNetRef;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        try
        {
            List<LicenseDto> all = await GpuPhotosService.GetAllLicensesAsync();
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        foreach(StagedPhoto p in _staged)
        {
            ...
            try
            {
                ...
                await JS.InvokeAsync<object>("MarechaiGpuPhotoUpload.applyPendingPhotoImage",
    ...
    async Task OnFilesPicked()
    {
        try
        {
            ...
            await JS.InvokeAsync<object>("MarechaiGpuPhotoUpload.uploadGpuPhotos",
                                          ApiAssetUrls.BaseUrl, token, GpuId, _fileInputId, _dotNetRef);
        }
        catch(Exception ex)
        {
    ...
    async Task RemoveStagedPhotoAsync(string clientGuid)
    {
        ...
        {
            try { await JS.InvokeVoidAsync("MarechaiGpuPhotoUpload.cancelUpload", p.ClientGuid); } catch { /* ignored */ }
        }

        // Delete the server-side pending file (no-op if upload never completed).
        if(p.ServerGuid != Guid.Empty)
        {
            await GpuPhotosService.DeletePendingPhotoAsync(p.ServerGuid);
        }
    ...
    public Task OnPhotoUploadStarted(string clientGuid, string fileName, long sizeBytes)
    {
        _staged.Add(new StagedPhoto
        {
            ...
            UploadPercent = 0,
            ImgElementId  = $"gpu-photo-thumb-{Guid.NewGuid():N}"
        });
        StateHasChanged();
    ...
    async Task SubmitAsync()
    {
        ...
        try
        {
            ...
            {
                EntityType          = (int?)SuggestionEntityType.GpuPhoto,
                EntityId            = GpuId,
    ...
    async Task OnCancelAsync()
    {
        ...
        foreach(StagedPhoto p in _staged.ToList())
        {
            ...
            if(p.ServerGuid != Guid.Empty)
            {
                try { await GpuPhotosService.DeletePendingPhotoAsync(p.ServerGuid); } catch { /* ignored */ }
            }
        }
