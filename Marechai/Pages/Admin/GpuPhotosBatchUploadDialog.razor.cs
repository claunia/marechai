...
public partial class GpuPhotosBatchUploadDialog : ComponentBase, IAsyncDisposable
{
    ...
    [Parameter] public string             GpuName  { get; set; } = "";
    ...
    readonly string            _fileInputId = $"gp-batch-input-{Guid.NewGuid():N}";

    DotNetObjectReference<GpuPhotosBatchUploadDialog> _dotNetRef;

    Guid?                                          _jobId;
    int                                            _jobTotal;
    int                                            _jobProcessed;
    Guid?                                          _currentPendingId;
    List<AdminGpuPhotoBatchJobItemResultDto>       _jobResults = new();
    ...
    {
        try { await JS.InvokeVoidAsync("MarechaiGpuPhotoBatchUpload.clickInput", _fileInputId); }
        catch { /* best-effort */ }
    }
    ...
    async Task OnFilesPicked()
    {
        try
        {
            ...
            await JS.InvokeAsync<object>("MarechaiGpuPhotoBatchUpload.uploadBatch",
                                         ApiAssetUrls.BaseUrl, token, GpuId, _fileInputId,
    ...
    async Task RemoveStagedAsync(string clientGuid)
    {
        ...
        {
            try { await JS.InvokeVoidAsync("MarechaiGpuPhotoBatchUpload.cancelUpload", c.ClientGuid); }
            catch { /* ignored */ }
        }
    ...
    async Task SubmitAsync()
    {
        ...
        var request = new AdminGpuPhotoBatchCommitRequestDto
        {
            ...
            Items = _staged
                    ...
                    .Select(s => new AdminGpuPhotoBatchCommitItemDto
                     {
        ...
        _jobResults       = job.Results ?? new List<AdminGpuPhotoBatchJobItemResultDto>();
    ...
    async Task PollLoopAsync(CancellationToken ct)
    {
        try
        {
            while(!ct.IsCancellationRequested && _jobId.HasValue)
            {
                ...
                AdminGpuPhotoBatchJobStatusDto status = await PhotosService.GetAdminBatchStatusAsync(_jobId.Value);
                ...
                _jobResults       = status.Results ?? new List<AdminGpuPhotoBatchJobItemResultDto>();
