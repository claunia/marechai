...
public partial class SoftwareVariant
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _creating = NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                     .ToLowerInvariant()
                                     .StartsWith("admin/software_variants/create", StringComparison.InvariantCulture);
        ...
        _editing = _creating ||
                   NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                    .ToLowerInvariant()
                                    .StartsWith("admin/software_variants/edit/", StringComparison.InvariantCulture);
    ...
    async void OnSaveClicked()
    {
        ...
        if(_unknownVersion)
            _model.Version = null;
        else if(string.IsNullOrWhiteSpace(_model.Version)) return;

        if(_unknownPartNumber)
            _model.PartNumber = null;
        else if(string.IsNullOrWhiteSpace(_model.PartNumber)) return;

        if(_unknownSerialNumber)
            _model.SerialNumber = null;
        else if(string.IsNullOrWhiteSpace(_model.SerialNumber)) return;
        ...
        else if(string.IsNullOrWhiteSpace(_model.CatalogueNumber)) return;

        if(_unknownParent)
            _model.ParentId = null;
        else if(_model.ParentId < 1) return;
    ...
    async Task ConfirmAddCompany()
    {
        ...
        await CompaniesBySoftwareVariantService.CreateAsync(_addingCompanyId.Value,
                                                            Id,
    ...
    void ShowCompanyDeleteModal(ulong itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the company {0} with role {1} from this software variant?"],
                          _currentCompanyBySoftwareVariant?.Company,
                          _currentCompanyBySoftwareVariant?.Role);
    ...
    {
        if(_deletingCompanyBySoftwareVariant) await ConfirmDeleteCompanyByMachine();
    }

    async Task ConfirmDeleteCompanyByMachine()
    {
        if(_currentCompanyBySoftwareVariant is null) return;
