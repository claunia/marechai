...
public partial class CurrencyPegging
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _creating = NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                     .ToLowerInvariant()
                                     .StartsWith("admin/currency_pegging/create", StringComparison.InvariantCulture);
        ...
        _editing = _creating ||
                   NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                    .ToLowerInvariant()
                                    .StartsWith("admin/currency_pegging/edit/", StringComparison.InvariantCulture);
    ...
    {
        if(string.IsNullOrWhiteSpace(_model.SourceCode)) return;

        if(string.IsNullOrWhiteSpace(_model.DestinationCode)) return;

        if(_model.Start >= DateTime.UtcNow) return;

        if(_model.End >= DateTime.UtcNow || _model.End < _model.Start) return;

        if(_model.Ratio <= 0) return;
    ...
    void ValidateStart(ValidatorEventArgs e)
    {
        ...
        if((DateTime)e.Value <= DateTime.UtcNow) return;
    ...
    void ValidateEnd(ValidatorEventArgs e)
    {
        ...
        if(!((DateTime?)e.Value < _model.Start)) return;
