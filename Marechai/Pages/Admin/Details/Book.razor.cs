...
public partial class Book
{
    ...
    ushort ScanColorSpace
    {
        get
        {
            if(_selectedScan.ColorSpace is null) return 0;
    ...
    ushort ScanResolutionUnit
    {
        get
        {
            if(_selectedScan.ResolutionUnit is null) return 0;
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _scans                 = await BookScansService.GetGuidsByBookAsync(Id);

        _editing = _creating ||
                   NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                    .ToLowerInvariant()
                                    .StartsWith("admin/books/edit/", StringComparison.InvariantCulture);
    ...
    async void OnSaveClicked()
    {
        ...
        else if(string.IsNullOrWhiteSpace(_model.NativeTitle)) return;

        if(_unknownCountry)
            _model.CountryId = null;
        else if(_model.CountryId < 0) return;

        if(_unknownPages)
            _model.Pages = null;
        else if(_model.Pages < 1) return;

        if(_unknownEdition)
            _model.Edition = null;
        else if(_model.Edition < 1) return;

        if(_unknownPublished)
            _model.Published = null;
        else if(_model.Published?.Date >= DateTime.UtcNow.Date) return;

        if(_unknownIsbn)
            _model.Isbn = null;
        else if(string.IsNullOrWhiteSpace(_model.Isbn)) return;

        // Convert ISBN-10 to ISBN-13
        if(_model.Isbn?.Length == 10)
        {
            var newIsbn = new char[13];
    ...
    async Task ConfirmAddCompany()
    {
        ...
        await CompaniesByBookService.CreateAsync(_addingCompanyId.Value,
                                                 Id,
    ...
    void ShowCompanyDeleteModal(long itemId)
    {
        ...
        _deleteText = string.Format(L["Are you sure you want to delete the company {0} with role {1} from this book?"],
                                    _currentCompanyByBook?.Company,
                                    _currentCompanyByBook?.Role);
    ...
    async void ConfirmDelete()
    {
        ...
        else if(_deletingScan) await ConfirmDeleteScan();
    }

    async Task ConfirmDeleteCompanyByBook()
    {
        if(_currentCompanyByBook is null) return;
    ...
    {
        if(_addingMachineFamilyId is null || _addingMachineFamilyId <= 0)
        {
            CancelAddFamily();
        ...
        await BooksByMachineFamilyService.CreateAsync(_addingMachineFamilyId.Value,
                                                      Id,
                                                      (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    {
        if(_currentBookByMachineFamily is null) return;
    ...
    {
        if(_addingMachineId is null || _addingMachineId <= 0)
        {
            CancelAddMachine();
        ...
        await BooksByMachineService.CreateAsync(_addingMachineId.Value,
                                                Id,
                                                (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    {
        if(_currentBookByMachine is null) return;
    ...
    {
        if(_addingPersonId is null || _addingPersonId <= 0)
        {
            CancelAddPerson();
        ...
        await PeopleByBookService.CreateAsync(_addingPersonId.Value,
                                              Id,
                                              _addingPersonRoleId,
                                              (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    void ShowPersonDeleteModal(long itemId)
    {
        ...
        _deleteText = string.Format(L["Are you sure you want to delete the person {0} with role {1} from this book?"],
                                    _currentPersonByBook?.FullName,
                                    _currentPersonByBook?.Role);
    ...
    {
        if(_currentPersonByBook is null) return;
    ...
    {
        if(!(e.Value is DateTime item) || item.Year <= 1816 || item >= DateTime.UtcNow)
    ...
    {
        if(e.Value is double item && item > 0)
            e.Status = ValidationStatus.Success;
        else
            e.Status = ValidationStatus.Error;
    }
    ...
    {
        if(e.Value is uint item && item > 0)
            e.Status = ValidationStatus.Success;
        else
            e.Status = ValidationStatus.Error;
    }
    ...
    async Task UploadFile()
    {
        ...
        try
        {
            ...
            while((count = await fs.ReadAsync(buffer, 0, buffer.Length)) != 0)
            {
                ...
                if(!(progress > lastProgress + 0.01)) continue;
        ...
        if(processIdentify.ExitCode != 0 || string.IsNullOrWhiteSpace(identifyOutput))
        {
            _uploading          = false;
            _uploadError        = true;
        ...
        string[] pieces = identifyOutput.Substring(tmpPath.Length).Split(" ", StringSplitOptions.RemoveEmptyEntries);
        ...
        try
        {
            ...
            if(exif?.Length >= 1) exif[0].ToViewModel(_selectedScan);
        ...
        string originalFilePath = Path.Combine(Host.WebRootPath,
                                               "assets",
                                               "scans",
                                               "books",
                                               "originals",
...
#pragma warning disable 4014
        Task.Run(() => photos.ConversionWorker(Host.WebRootPath,
                                               _selectedScan.Id,
                                               originalFilePath,
                                               _imageFormat,
                                               true,
...
#pragma warning restore 4014
    ...
    async Task OnFinishedAll(bool result)
    {
        try
        {
            await BookScansService.CreateAsync(_selectedScan, (await UserManager.GetUserAsync(_authState.User)).Id);
            _addToDatabase = true;
        }
        catch(Exception e)
        {
    ...
    {
        _selectedScan = await BookScansService.GetAsync(scanId);
    ...
    async Task ShowScanDeleteModal(Guid scanId)
    {
        ...
        _deletingScan = true;
        _deleteTitle  = L["Delete scan from this book"];

        _deleteText = _selectedScan.Page.HasValue
                          ? string.Format(L["Are you sure you want to delete the scan type {0} for page {1} from this book?"],
                                          _selectedScan.Type,
                                          _selectedScan.Page)
                          : string.Format(L["Are you sure you want to delete the scan type {0} from this book?"],
                                          _selectedScan.Type);
    ...
    async Task ConfirmDeleteScan()
    {
        ...
        // TODO: Delete files
        await BookScansService.DeleteAsync(_selectedScan.Id, (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    async void OnSaveScanClicked()
    {
        ...
        else if(string.IsNullOrWhiteSpace(_selectedScan.Author)) return;

        if(_unknownScanScannerManufacturer)
            _selectedScan.ScannerModel = null;
        else if(string.IsNullOrWhiteSpace(_selectedScan.ScannerModel)) return;

        if(_unknownScanScannerModel)
            _selectedScan.ScannerManufacturer = null;
        else if(string.IsNullOrWhiteSpace(_selectedScan.ScannerManufacturer)) return;

        if(_unknownScanColorSpace)
            _selectedScan.ColorSpace = null;
        else if(!_selectedScan.ColorSpace.HasValue) return;

        if(_unknownScanCreationDate)
            _selectedScan.CreationDate = null;
        else if(_selectedScan.CreationDate > DateTime.UtcNow) return;

        if(_unknownScanExifVersion)
            _selectedScan.ExifVersion = null;
        else if(string.IsNullOrWhiteSpace(_selectedScan.ExifVersion)) return;

        if(_unknownScanHorizontalResolution)
            _selectedScan.HorizontalResolution = null;
        else if(!_selectedScan.HorizontalResolution.HasValue) return;

        if(_unknownScanResolutionUnit)
            _selectedScan.ResolutionUnit = null;
        else if(!_selectedScan.ResolutionUnit.HasValue) return;

        if(_unknownScanSoftwareUsed)
            _selectedScan.SoftwareUsed = null;
        else if(string.IsNullOrWhiteSpace(_selectedScan.SoftwareUsed)) return;

        if(_unknownScanVerticalResolution)
            _selectedScan.VerticalResolution = null;
        else if(!_selectedScan.VerticalResolution.HasValue) return;

        if(_unknownScanComments)
            _selectedScan.Comments = null;
        else if(string.IsNullOrWhiteSpace(_selectedScan.Comments)) return;

        if(_unknownScanPage)
            _selectedScan.Page = null;
        else if(!_selectedScan.Page.HasValue || _selectedScan.Page == 0) return;

        await BookScansService.UpdateAsync(_selectedScan, (await UserManager.GetUserAsync(_authState.User)).Id);
