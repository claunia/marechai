...
public partial class Person
{
    ...
    async void OnSaveClicked()
    {
        if(_unknownAlias)
            _model.Alias = null;
        else if(string.IsNullOrWhiteSpace(_model.Alias)) return;

        if(_unknownCountry)
            _model.CountryOfBirthId = null;
        else if(_model.CountryOfBirthId < 0) return;

        if(_model.BirthDate.Date >= DateTime.UtcNow.Date) return;

        if(_unknownDeathDate)
            _model.DeathDate = null;
        else if(_model.DeathDate?.Date >= DateTime.UtcNow.Date)
            return;
        else if(_model.DeathDate?.Date <= _model.BirthDate.Date) return;
        ...
        else if(string.IsNullOrWhiteSpace(_model.DisplayName)) return;
        ...
        if(_unknownSurname)
            _model.Surname = null;
        else if(string.IsNullOrWhiteSpace(_model.Surname)) return;
        ...
        if(_unknownWebpage)
            _model.Webpage = null;
        else if(string.IsNullOrWhiteSpace(_model.Webpage)) return;

        if(_unknownName && !_unknownSurname || !_unknownName && _unknownSurname) return;

        // TODO: Show error here
        if(_unknownName && _unknownSurname && _unknownAlias && _unknownDisplayName) return;
    ...
    void ValidateName(ValidatorEventArgs e)
    {
        ...
        if(!string.IsNullOrWhiteSpace(_model.Surname) && !_unknownSurname) return;
    ...
    void ValidateSurname(ValidatorEventArgs e)
    {
        ...
        if(surname.Length < 1 || surname.Length > 256)
        {
    ...
    void ValidateBirthDate(ValidatorEventArgs e)
    {
        ...
        if(_unknownDeathDate || !_model.DeathDate.HasValue) return;

        if(date.Date < _model.DeathDate?.Date) return;
    ...
    void ValidateDeathDate(ValidatorEventArgs e)
    {
        ...
        if(date.Date > _model.BirthDate.Date) return;
