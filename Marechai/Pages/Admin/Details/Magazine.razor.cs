...
public partial class Magazine
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _editing = _creating ||
                   NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                    .ToLowerInvariant()
                                    .StartsWith("admin/magazines/edit/", StringComparison.InvariantCulture);
    ...
    async void OnSaveClicked()
    {
        ...
        else if(_model.FirstPublication?.Date >= DateTime.UtcNow.Date) return;

        if(_unknownIssn)
            _model.Issn = null;
        else if(string.IsNullOrWhiteSpace(_model.Issn)) return;
    ...
    async Task ConfirmAddCompany()
    {
        ...
        await CompaniesByMagazineService.CreateAsync(_addingCompanyId.Value,
                                                     Id,
    ...
    void ShowCompanyDeleteModal(long itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the company {0} with role {1} from this magazine?"],
                          _currentCompanyByMagazine?.Company,
                          _currentCompanyByMagazine?.Role);
    ...
    {
        if(_deletingCompanyByMagazine) await ConfirmDeleteCompanyByMachine();
    }

    async Task ConfirmDeleteCompanyByMachine()
    {
        if(_currentCompanyByMagazine is null) return;
