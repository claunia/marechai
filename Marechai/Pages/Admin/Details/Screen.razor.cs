...
public partial class Screen
{
    ...
    async void OnSaveClicked()
    {
        if(_unknownWidth)
            _model.Width = null;
        else if(_model.Width < 0) return;

        if(_unknownHeight)
            _model.Height = null;
        else if(_model.Height < 0) return;

        if(_unknownType)
            _model.Type = null;
        else if(string.IsNullOrWhiteSpace(_model.Type)) return;

        if(_unknownColors)
            _model.EffectiveColors = null;
        else if(_model.EffectiveColors < 0) return;
    ...
    async Task ConfirmAddResolution()
    {
        ...
        await ResolutionsByScreenService.CreateAsync(_addingResolutionId.Value,
                                                     Id,
                                                     (await UserManager.GetUserAsync(_authState.User)).Id);
