...
public partial class License
{
    ...
    async void OnSaveClicked()
    {
        if(_unknownText)
            _model.Text = null;
        else if(string.IsNullOrWhiteSpace(_model.Text)) return;

        if(string.IsNullOrWhiteSpace(_model.Text) || _model.Name?.Length > 255) return;

        if(string.IsNullOrWhiteSpace(_model.SPDX) || _model.SPDX?.Length > 255) return;

        if(_unknownLink)
            _model.Link = null;
        else if(string.IsNullOrWhiteSpace(_model.Link) || _model.Link?.Length > 512) return;
