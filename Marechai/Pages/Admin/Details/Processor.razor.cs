...
public partial class Processor
{
    ...
    async void OnSaveClicked()
    {
        ...
        else if(string.IsNullOrWhiteSpace(_model.ModelCode)) return;

        if(_unknownIntroduced)
            _model.Introduced = null;
        else if(_prototype)
            _model.Introduced = new DateTime(1000, 1, 1);
        else if(_model.Introduced >= DateTime.UtcNow.Date) return;

        if(_unknownInstructionSet)
            _model.InstructionSetId = null;
        else if(_model.InstructionSetId < 0) return;

        if(_unknownSpeed)
            _model.Speed = null;
        else if(_model.Speed < 0) return;

        if(_unknownPackage)
            _model.Package = null;
        else if(string.IsNullOrWhiteSpace(_model.Package)) return;

        if(_unknownGprs)
            _model.Gprs = null;
        else if(_model.Gprs < 0) return;

        if(_unknownGprSize)
            _model.GprSize = null;
        else if(_model.GprSize < 0) return;

        if(_unknownFprs)
            _model.Fprs = null;
        else if(_model.Fprs < 0) return;

        if(_unknownFprSize)
            _model.FprSize = null;
        else if(_model.FprSize < 0) return;

        if(_unknownSimdRegisters)
            _model.SimdRegisters = null;
        else if(_model.SimdRegisters < 0) return;

        if(_unknownSimdSize)
            _model.SimdSize = null;
        else if(_model.SimdSize < 0) return;

        if(_unknownCores)
            _model.Cores = null;
        else if(_model.Cores < 1) return;
        ...
        if(_unknownProcess)
            _model.Process = null;
        else if(string.IsNullOrWhiteSpace(_model.Process)) return;

        if(_unknownProcessNm)
            _model.ProcessNm = null;
        else if(_model.ProcessNm < 1) return;

        if(_unknownDieSize)
            _model.DieSize = null;
        else if(_model.DieSize < 1) return;

        if(_unknownTransistors)
            _model.Transistors = null;
        else if(_model.Transistors < 0) return;

        if(_unknownDataBus)
            _model.DataBus = null;
        else if(_model.DataBus < 1) return;

        if(_unknownAddressBus)
            _model.AddrBus = null;
        else if(_model.AddrBus < 1) return;

        if(_unknownL1Instruction)
            _model.L1Instruction = null;
        else if(_model.L1Instruction < 0) return;

        if(_unknownL1Data)
            _model.L1Data = null;
        else if(_model.L1Data < 0) return;

        if(_unknownL2)
            _model.L2 = null;
        else if(_model.L2 < 0) return;

        if(_unknownL3)
            _model.FprSize = null;
        else if(_model.L3 < 0) return;
    ...
    {
        if(_currentInstructionByMachine is null) return;
        ...
        await InstructionSetExtensionsByProcessorService.DeleteAsync(_currentInstructionByMachine.Id,
                                                                     (await UserManager.GetUserAsync(_authState.User))
                                                                    .Id);
    ...
    {
        if(_addingExtensionId is null || _addingExtensionId <= 0)
        {
            CancelAddExtension();
        ...
        await InstructionSetExtensionsByProcessorService.CreateAsync(Id,
                                                                     _addingExtensionId.Value,
                                                                     (await UserManager.GetUserAsync(_authState.User))
                                                                    .Id);
