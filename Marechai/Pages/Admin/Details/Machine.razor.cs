...
public partial class Machine
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _model           = _creating ? new MachineDto() : await Service.GetAsync(Id);
    ...
    async void OnSaveClicked()
    {
        ...
        else if(_model.FamilyId < 0) return;
        ...
        if(_unknownModel)
            _model.Model = null;
        else if(string.IsNullOrWhiteSpace(_model.Model)) return;
    ...
    void ShowGpuDeleteModal(long itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the graphical processing unit {0} manufactured by {1} from this machine?"],
                          _currentGpuByMachine?.Name,
                          _currentGpuByMachine?.CompanyName);
    ...
    async void ConfirmDelete()
    {
        ...
        else if(_deletingScreenByMachine) await ConfirmDeleteScreenByMachine();
    }

    async Task ConfirmDeleteGpuByMachine()
    {
        if(_currentGpuByMachine is null) return;
    ...
    {
        if(_addingGpuId is null || _addingGpuId <= 0)
        {
            CancelAddGpu();
        ...
        await GpusByMachineService.CreateAsync(_addingGpuId.Value,
                                               Id,
                                               (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    void ShowSoundDeleteModal(long itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the sound synthesizer {0} manufactured by {1} from this machine?"],
                          _currentSoundByMachine?.Name,
                          _currentSoundByMachine?.CompanyName);
    ...
    {
        if(_currentSoundByMachine is null) return;
    ...
    {
        if(_addingSoundId is null || _addingSoundId <= 0)
        {
            CancelAddSound();
        ...
        await SoundSynthsByMachineService.CreateAsync(_addingSoundId.Value,
                                                      Id,
                                                      (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    void ShowCpuDeleteModal(long itemId)
    {
        ...
        string speed = _currentCpuByMachine?.Speed == null
                           ? L["Unknown (processor by machine speed)"]
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the processor {0} with speed {2} manufactured by {1} from this machine?"],
                          _currentCpuByMachine?.Name,
                          _currentCpuByMachine?.CompanyName,
                          speed);
    ...
    {
        if(_currentCpuByMachine is null) return;
    ...
    {
        if(_addingCpuId is null || _addingCpuId <= 0)
        {
            CancelAddCpu();
        ...
        await ProcessorsByMachineService.CreateAsync(_addingCpuId.Value,
                                                     Id,
    ...
    void ShowMemoryDeleteModal(long itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the memory type {0} with usage {1} from this machine?"],
                          _currentMemoryByMachine?.Type,
                          _currentMemoryByMachine?.Usage);
    ...
    {
        if(_currentMemoryByMachine is null) return;
    ...
    async Task ConfirmAddMemory()
    {
        ...
        await MemoriesByMachineService.CreateAsync(Id,
                                                   (MemoryType)_addingMemoryType,
    ...
    void ShowStorageDeleteModal(long itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the storage type {0} with interface {1} from this machine?"],
                          _currentStorageByMachine?.Type,
                          _currentStorageByMachine?.Interface);
    ...
    {
        if(_currentStorageByMachine is null) return;
    ...
    async Task ConfirmAddStorage()
    {
        ...
        await StorageByMachineService.CreateAsync(Id,
                                                  (StorageType)_addingStorageType,
    ...
    {
        if(_currentScreenByMachine is null) return;
    ...
    {
        if(_addingScreenId is null || _addingScreenId <= 0)
        {
            CancelAddScreen();
        ...
        await ScreensByMachineService.CreateAsync(_addingScreenId.Value,
                                                  Id,
                                                  (await UserManager.GetUserAsync(_authState.User)).Id);
