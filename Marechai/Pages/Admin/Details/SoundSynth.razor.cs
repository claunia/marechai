...
public partial class SoundSynth
{
    ...
    async void OnSaveClicked()
    {
        ...
        if(_unknownVoices)
            _model.Voices = null;
        else if(_model.Voices < 0) return;

        if(_unknownSampleRate)
            _model.Frequency = null;
        else if(_model.Frequency < 0) return;

        if(_unknownSampleResolution)
            _model.Depth = null;
        else if(_model.Depth < 0) return;

        if(_unknownSquareWaveChannels)
            _model.SquareWave = null;
        else if(_model.SquareWave < 0) return;

        if(_unknownWhiteNoiseChannels)
            _model.WhiteNoise = null;
        else if(_model.WhiteNoise < 0) return;

        if(_unknownType)
            _model.Type = null;
        else if(_model.Type < 0) return;
