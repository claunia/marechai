...
public partial class MagazineIssue
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        _editing = _creating ||
                   NavigationManager.ToBaseRelativePath(NavigationManager.Uri)
                                    .ToLowerInvariant()
                                    .StartsWith("admin/magazine_issues/edit/", StringComparison.InvariantCulture);
    ...
    async void OnSaveClicked()
    {
        ...
        else if(string.IsNullOrWhiteSpace(_model.NativeCaption)) return;
        ...
        else if(_model.IssueNumber < 1) return;
        ...
        else if(string.IsNullOrWhiteSpace(_model.ProductCode)) return;

        if(string.IsNullOrWhiteSpace(_model.Caption)) return;
    ...
    async Task ConfirmAddFamily()
    {
        ...
        await MagazinesByMachineFamilyService.CreateAsync(_addingMachineFamilyId.Value,
                                                          Id,
    ...
    {
        if(_currentMagazineByMachineFamily is null) return;
    ...
    async Task ConfirmAddMachine()
    {
        ...
        await MagazinesByMachineService.CreateAsync(_addingMachineId.Value,
                                                    Id,
                                                    (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    {
        if(_currentMagazineByMachine is null) return;
    ...
    async Task ConfirmAddPerson()
    {
        ...
        await PeopleByMagazineService.CreateAsync(_addingPersonId.Value,
                                                  Id,
                                                  _addingPersonRoleId,
                                                  (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    void ShowPersonDeleteModal(long itemId)
    {
        ...
        _deleteText =
            string.Format(L["Are you sure you want to delete the person {0} with role {1} from this magazine?"],
                          _currentPersonByMagazine?.FullName,
                          _currentPersonByMagazine?.Role);
    ...
    {
        if(_currentPersonByMagazine is null) return;
...
#pragma warning restore 4014
    ...
    async Task OnFinishedAll(bool result)
    {
        try
        {
            await MagazineScansService.CreateAsync(_selectedScan, (await UserManager.GetUserAsync(_authState.User)).Id);
    ...
    async Task ShowScanDeleteModal(Guid scanId)
    {
        ...
        _deleteText = _selectedScan.Page.HasValue
                          ? string.Format(L["Are you sure you want to delete the scan type {0} for page {1} from this magazine?"],
                                          _selectedScan.Type,
                                          _selectedScan.Page)
                          : string.Format(L["Are you sure you want to delete the scan type {0} from this magazine?"],
                                          _selectedScan.Type);
    ...
    async Task ConfirmDeleteScan()
    {
        ...
        // TODO: Delete files
        await MagazineScansService.DeleteAsync(_selectedScan.Id, (await UserManager.GetUserAsync(_authState.User)).Id);
