...
public partial class ProcessorPhotosBatchUploadDialog : ComponentBase, IAsyncDisposable
{
    ...
    readonly string            _fileInputId = $"pp-batch-input-{Guid.NewGuid():N}";

    DotNetObjectReference<ProcessorPhotosBatchUploadDialog> _dotNetRef;

    Guid?                                              _jobId;
    int                                                _jobTotal;
    int                                                _jobProcessed;
    Guid?                                              _currentPendingId;
    List<AdminProcessorPhotoBatchJobItemResultDto>     _jobResults = new();
    ...
    {
        try { await JS.InvokeVoidAsync("MarechaiProcessorPhotoBatchUpload.clickInput", _fileInputId); }
        catch { /* best-effort */ }
    }
    ...
    async Task OnFilesPicked()
    {
        try
        {
            ...
            await JS.InvokeAsync<object>("MarechaiProcessorPhotoBatchUpload.uploadBatch",
                                         ApiAssetUrls.BaseUrl, token, ProcessorId, _fileInputId,
    ...
    async Task RemoveStagedAsync(string clientGuid)
    {
        ...
        {
            try { await JS.InvokeVoidAsync("MarechaiProcessorPhotoBatchUpload.cancelUpload", c.ClientGuid); }
            catch { /* ignored */ }
        }
    ...
    async Task SubmitAsync()
    {
        ...
        var request = new AdminProcessorPhotoBatchCommitRequestDto
        {
            ...
            Items = _staged
                    ...
                    .Select(s => new AdminProcessorPhotoBatchCommitItemDto
                     {
        ...
        _jobResults       = job.Results ?? new List<AdminProcessorPhotoBatchJobItemResultDto>();
    ...
    async Task PollLoopAsync(CancellationToken ct)
    {
        try
        {
            while(!ct.IsCancellationRequested && _jobId.HasValue)
            {
                ...
                AdminProcessorPhotoBatchJobStatusDto status = await PhotosService.GetAdminBatchStatusAsync(_jobId.Value);
                ...
                _jobResults       = status.Results ?? new List<AdminProcessorPhotoBatchJobItemResultDto>();
