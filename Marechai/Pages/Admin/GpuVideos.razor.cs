...
public partial class GpuVideos
{
    ...
    {
        GpuDto gpu = await GpusService.GetByIdAsync(Id);
        _gpuName = gpu?.Name;
        await LoadDataAsync();
    }
    ...
    {
        _isLoading = true;
        _videos    = await GpusService.GetVideosByGpuAsync(Id);
        _isLoading = false;
    }
    ...
    async Task LinkVideoAsync()
    {
        ...
        try
        {
            (GpuVideoDto dto, string error) = await GpusService.CreateVideoAsync(Id,
    ...
    async Task OnTitleChangedAsync(GpuVideoDto video, string newTitle)
    {
        if(video.Title == newTitle) return;

        (bool succeeded, string error) =
            await GpusService.UpdateVideoTitleAsync(video.Id!.Value, newTitle);
    ...
    async Task ConfirmDeleteAsync(GpuVideoDto video)
    {
        ...
        (bool succeeded, string errorMessage) =
            await GpusService.DeleteVideoAsync(video.Id ?? 0);
