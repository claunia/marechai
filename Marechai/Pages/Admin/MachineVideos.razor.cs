...
public partial class MachineVideos
{
    ...
    {
        MachineDto machine = await MachinesService.GetByIdAsync(Id);
    ...
    {
        _isLoading = true;
        _videos    = await MachinesService.GetVideosByMachineAsync(Id);
        _isLoading = false;
    }
    ...
    async Task LinkVideoAsync()
    {
        ...
        try
        {
            (MachineVideoDto dto, string error) = await MachinesService.CreateVideoAsync(Id,
    ...
    async Task OnTitleChangedAsync(MachineVideoDto video, string newTitle)
    {
        if(video.Title == newTitle) return;

        (bool succeeded, string error) =
            await MachinesService.UpdateVideoTitleAsync(video.Id!.Value, newTitle);
    ...
    async Task ConfirmDeleteAsync(MachineVideoDto video)
    {
        ...
        (bool succeeded, string errorMessage) =
            await MachinesService.DeleteVideoAsync(video.Id ?? 0);
