...
public partial class MachinePhotosBatchUploadDialog : ComponentBase, IAsyncDisposable
{
    ...
    readonly string            _fileInputId = $"mp-batch-input-{Guid.NewGuid():N}";

    DotNetObjectReference<MachinePhotosBatchUploadDialog> _dotNetRef;

    Guid?                                              _jobId;
    int                                                _jobTotal;
    int                                                _jobProcessed;
    Guid?                                              _currentPendingId;
    List<AdminMachinePhotoBatchJobItemResultDto>       _jobResults = new();
    ...
    {
        try { await JS.InvokeVoidAsync("MarechaiMachinePhotoBatchUpload.clickInput", _fileInputId); }
        catch { /* best-effort */ }
    }
    ...
    async Task OnFilesPicked()
    {
        try
        {
            ...
            await JS.InvokeAsync<object>("MarechaiMachinePhotoBatchUpload.uploadBatch",
                                         ApiAssetUrls.BaseUrl, token, MachineId, _fileInputId,
    ...
    async Task RemoveStagedAsync(string clientGuid)
    {
        ...
        {
            try { await JS.InvokeVoidAsync("MarechaiMachinePhotoBatchUpload.cancelUpload", c.ClientGuid); }
            catch { /* ignored */ }
        }
    ...
    async Task SubmitAsync()
    {
        ...
        var request = new AdminMachinePhotoBatchCommitRequestDto
        {
            ...
            Items = _staged
                    ...
                    .Select(s => new AdminMachinePhotoBatchCommitItemDto
                     {
        ...
        _jobResults       = job.Results ?? new List<AdminMachinePhotoBatchJobItemResultDto>();
    ...
    async Task PollLoopAsync(CancellationToken ct)
    {
        try
        {
            while(!ct.IsCancellationRequested && _jobId.HasValue)
            {
                ...
                AdminMachinePhotoBatchJobStatusDto status = await PhotosService.GetAdminBatchStatusAsync(_jobId.Value);
                ...
                _jobResults       = status.Results ?? new List<AdminMachinePhotoBatchJobItemResultDto>();
