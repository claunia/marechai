...
public partial class Search : IAsyncDisposable
{
    ...
    readonly string               _sentinelId  = $"tablets-search-sentinel-{Guid.NewGuid():N}";
    readonly string               _scrollerId  = $"tablets-search-scroll-{Guid.NewGuid():N}";
    readonly List<MachineDto>     _tablets = [];
    char?                         _character;
    bool                          _initialized;
    bool                          _loadingMore;
    ...
    bool HasMore => _tablets.Count < _total;
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if(!_initialized)
        {
            ...
            _tablets.AddRange(firstPageTask.Result);
            _initialized = true;
    ...
    public async Task OnSentinelVisibleAsync()
    {
        ...
        try
        {
            List<MachineDto> next = await FetchBatchAsync(_tablets.Count, _pageSize);
            _tablets.AddRange(next);
    ...
    Task<int> GetCurrentCountAsync()
    {
        ...
        if(_character.HasValue) return Service.GetTabletsByLetterCountAsync(_character.Value);

        if(Year.HasValue) return Service.GetTabletsByYearCountAsync(Year.Value);

        return Service.GetTabletsCountAsync();
    }
    ...
    Task<List<MachineDto>> FetchBatchAsync(int skip, int take)
    {
        ...
        if(_character.HasValue) return Service.GetTabletsByLetterAsync(_character.Value, skip, take);

        if(Year.HasValue) return Service.GetTabletsByYearAsync(Year.Value, skip, take);

        return Service.GetTabletsAsync(skip, take);
    }
