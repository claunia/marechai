...
/// <summary>
///     Suggestion metadata for the <see cref="DocumentDto" /> entity. Exposes 7 directly
...
///     Mirrors <see cref="BookSuggestionMetadata" /> minus cover/ISBN/Pages/Edition/
///     Previous/Source fields (Documents have no cover image and a leaner scalar surface).
/// </summary>
public sealed class DocumentSuggestionMetadata : SuggestionMetadata
{
    ...
    static DocumentSuggestionMetadata() => SuggestionMetadataRegistry.Register(new DocumentSuggestionMetadata());
    ...
    public override SuggestionEntityType EntityType => SuggestionEntityType.Document;
    ...
    public override Dictionary<string, object> ExtractCurrentValues(object currentDto)
    {
        ...
        if(currentDto is not DocumentDto d) return result;

        result[FieldTitle]              = d.Title;
        result[FieldNativeTitle]        = d.NativeTitle;
        result[FieldSortTitle]          = d.SortTitle;
        result[FieldPublished]          = d.Published?.UtcDateTime.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
        result[FieldPublishedPrecision] = d.PublishedPrecision;
        result[FieldCountryId]          = d.CountryId;
        result[FieldInternetArchiveUrl] = d.InternetArchiveUrl;
    ...
    ///     <c>Marechai.Server.Suggestions.DocumentSuggestionApplier.TryParseJunctionKey</c>.
    /// </summary>
