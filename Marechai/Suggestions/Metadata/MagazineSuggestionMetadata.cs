...
/// <summary>
///     Suggestion metadata for the <see cref="MagazineDto" /> entity. Exposes 9 directly
///     editable scalar fields PLUS dynamic junction operation keys
///     (<c>companies.add.&lt;uuid&gt;</c> and <c>companies.remove.&lt;row_id&gt;</c>) for the
///     single suggestable junction table (<c>CompaniesByMagazine</c>). Mirrors
///     <see cref="DocumentSuggestionMetadata" /> minus the People/Machines/MachineFamilies
///     junctions (People belong to <c>MagazineIssue</c>, not Magazine), plus the
///     Magazine-specific <c>Issn</c>/<c>FirstPublication</c>/<c>FirstPublicationPrecision</c>
///     scalars (and minus the Document-only <c>InternetArchiveUrl</c>).
/// </summary>
public sealed class MagazineSuggestionMetadata : SuggestionMetadata
{
    ...
    static readonly HashSet<string> s_junctionGroups = new(StringComparer.Ordinal) { GroupCompanies };
    ...
    static readonly IReadOnlyList<SuggestionFieldDescriptor> s_fields = new SuggestionFieldDescriptor[]
    {
        ...
        new(FieldIssn,                      "ISSN",                        SuggestionFieldKind.Text, MaxLength: 8),
        new(FieldFirstPublication,          "First publication",           SuggestionFieldKind.Date),
        new(FieldFirstPublicationPrecision, "First publication precision", SuggestionFieldKind.Enum, EnumOptions: PrecisionOptions)
    };

    static MagazineSuggestionMetadata() => SuggestionMetadataRegistry.Register(new MagazineSuggestionMetadata());
    ...
    public override SuggestionEntityType EntityType => SuggestionEntityType.Magazine;
    ...
    public override Dictionary<string, object> ExtractCurrentValues(object currentDto)
    {
        ...
        if(currentDto is not MagazineDto m) return result;

        result[FieldTitle]                     = m.Title;
        result[FieldNativeTitle]               = m.NativeTitle;
        result[FieldSortTitle]                 = m.SortTitle;
        result[FieldPublished]                 = m.Published?.UtcDateTime.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
        result[FieldPublishedPrecision]        = m.PublishedPrecision;
        result[FieldCountryId]                 = m.CountryId;
        result[FieldIssn]                      = m.Issn;
        result[FieldFirstPublication]          = m.FirstPublication?.UtcDateTime.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
        result[FieldFirstPublicationPrecision] = m.FirstPublicationPrecision;
    ...
    ///     form <c>companies.{add,remove}.&lt;token&gt;</c>.
    /// </summary>
    ...
    /// <summary>
    ///     Override synthesises a localiser key for dynamic junction keys (e.g. <c>"Companies"</c>
    ///     for <c>companies.{add,remove}.&lt;token&gt;</c>); falls back to the static descriptor
    ...
    ///     <c>Marechai.Server.Suggestions.MagazineSuggestionApplier.TryParseJunctionKey</c>.
    /// </summary>
    ...
    public override string FormatDisplayValue(string fieldName, object value)
    {
        ...
        // Precision enum → label (both Published and FirstPublication share the same scale).
        if(fieldName == FieldPublishedPrecision || fieldName == FieldFirstPublicationPrecision)
        {
            int? i = ToInt(value);
            return i switch
            {
