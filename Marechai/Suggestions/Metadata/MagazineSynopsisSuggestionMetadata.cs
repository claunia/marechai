...
/// <summary>
///     Suggestion metadata for the per-language <c>MagazineSynopsis</c>. The suggestion's
///     <c>Subkey</c> holds the ISO-639-3 language code; the only suggestable field is the
///     markdown body itself.
/// </summary>
public sealed class MagazineSynopsisSuggestionMetadata : SuggestionMetadata
{
    /// <summary>Mirror of <c>MagazineSynopsisSuggestionApplier.FieldMarkdown</c>. KEEP IN SYNC.</summary>
    ...
    static MagazineSynopsisSuggestionMetadata() =>
        SuggestionMetadataRegistry.Register(new MagazineSynopsisSuggestionMetadata());
    ...
    public override SuggestionEntityType EntityType => SuggestionEntityType.MagazineSynopsis;
    ...
    public override Dictionary<string, object> ExtractCurrentValues(object currentDto)
    {
        ...
        // Magazine uses the shared DocumentSynopsisDto. The synopsis markdown is stored in
        // the Text property — there is no separate Html column on DocumentBaseSynopsis, so
        // the Markdown IS the canonical value.
