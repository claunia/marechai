...
/// <summary>
///     Suggestion metadata for the per-language <c>DocumentSynopsis</c>. The suggestion's
///     <c>Subkey</c> holds the ISO-639-3 language code; the only suggestable field is the
///     markdown body itself.
/// </summary>
public sealed class DocumentSynopsisSuggestionMetadata : SuggestionMetadata
{
    /// <summary>Mirror of <c>DocumentSynopsisSuggestionApplier.FieldMarkdown</c>. KEEP IN SYNC.</summary>
    ...
    static DocumentSynopsisSuggestionMetadata() =>
        SuggestionMetadataRegistry.Register(new DocumentSynopsisSuggestionMetadata());
    ...
    public override SuggestionEntityType EntityType => SuggestionEntityType.DocumentSynopsis;
    ...
    public override Dictionary<string, object> ExtractCurrentValues(object currentDto)
    {
        ...
        // Document uses the shared DocumentSynopsisDto. The synopsis markdown is stored in
