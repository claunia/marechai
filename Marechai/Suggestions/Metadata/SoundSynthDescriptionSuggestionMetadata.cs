...
/// <summary>
///     Suggestion metadata for the per-language <c>SoundSynthDescription</c>. The suggestion's
///     <c>Subkey</c> holds the ISO-639-3 language code; the only suggestable field is the
///     markdown body itself.
/// </summary>
public sealed class SoundSynthDescriptionSuggestionMetadata : SuggestionMetadata
{
    /// <summary>Mirror of <c>SoundSynthDescriptionSuggestionApplier.FieldMarkdown</c>. KEEP IN SYNC.</summary>
    ...
    {
        new(FieldMarkdown, "Description (Markdown)", SuggestionFieldKind.Markdown, MaxLength: 262144)
    };

    static SoundSynthDescriptionSuggestionMetadata() =>
        SuggestionMetadataRegistry.Register(new SoundSynthDescriptionSuggestionMetadata());
    ...
    public override SuggestionEntityType EntityType => SuggestionEntityType.SoundSynthDescription;
    ...
    public override Dictionary<string, object> ExtractCurrentValues(object currentDto)
    {
        ...
        if(currentDto is SoundSynthDescriptionDto d)
