...
public class ProcessorsService(Marechai.ApiClient.Client client, IndexNowService indexNow)
{
    ...
    ///     <c>FilterDefinition</c> shape; <see cref="ProcessorsController.ApplyFilters"/>
    ///     translates them into LINQ predicates.
    /// </summary>
    public async Task<List<ProcessorDto>> GetPagedAsync(int skip, int take, string sortBy, bool sortDescending,
                                         ...
                                         CancellationToken cancellationToken = default)
    {
        try
        {
            int? count = await client.Processors.Count.GetAsync(config =>
            {
                if(filters is { Count: > 0 }) config.QueryParameters.Filters = filters.ToArray();
            }, cancellationToken);
