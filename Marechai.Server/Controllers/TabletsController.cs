...
public class TabletsController(MarechaiContext context) : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public Task<List<MachineDto>> GetTabletsByLetterAsync(char c, [FromQuery] int? skip = null,
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public Task<List<MachineDto>> GetTabletsByYearAsync(int year, [FromQuery] int? skip = null,
                                                            [FromQuery] int? take = null,
                                                            CancellationToken cancellationToken = default)
    {
        IQueryable<Machine> ordered = context.Machines.Include(m => m.Company)
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public Task<List<MachineDto>> GetTabletsAsync([FromQuery] int? skip = null, [FromQuery] int? take = null,
                                                      CancellationToken cancellationToken = default)
    {
        IQueryable<Machine> ordered = context.Machines.Include(m => m.Company)
                                             .Where(m => m.Type == MachineType.Tablet)
