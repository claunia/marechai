...
[Route("/gpus/photos")]
[ApiController]
public class GpuPhotosController(MarechaiContext context, IConfiguration configuration,
                                 BatchUploadJobStore batchJobs) : ControllerBase
{
    ...
    /// <summary>
    ///     Stage a single image as part of an admin GPU-photo batch upload. The server
        ...
        [FromQuery] int gpuId)
    {
        ...
        await using(Stream stream = file.OpenReadStream())
        {
            guid = await PendingImageStore.StoreAsync(_assetRootPath, "gpus", extension,
                (byte)SuggestionEntityType.GpuPhoto, entityId: 0L, userId, file.ContentType, stream,
                parentEntityId: gpuId, _adminBatchAllowedExtensions, isAdminStaging: true,
                width: null, height: null);
        }

        string imagePath = await PendingImageStore.GetImagePathAsync(_assetRootPath, "gpus", guid);
        ...
        if(meta is not null)
        {
            meta.Width  = width;
            meta.Height = height;
            string sidecar = Path.Combine(_assetRootPath, "photos", "gpus", "pending",
                                          guid.ToString() + ".json");
        ...
        return Ok(new AdminPendingGpuPhotoUploadDto
        {
            Id              = guid,
            Extension       = extension.TrimStart('.'),
    ...
    /// <summary>
    ///     Delete a single admin-staged pending GPU-photo image before commit.
    ...
    /// <summary>
    ///     Commit a batch of admin-staged pending images into permanent <c>GpuPhoto</c>
    ///     rows. The endpoint returns 202 with a <c>jobId</c> immediately and processes the
    ///     batch on a background task; the client polls
    ///     <c>GET /gpus/photos/admin/batch/{jobId}/status</c> for progress and per-item
    ...
    [Authorize(Roles = "Admin,UberAdmin")]
    [ProducesResponseType(typeof(AdminGpuPhotoBatchJobStatusDto), StatusCodes.Status202Accepted)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<AdminGpuPhotoBatchJobStatusDto>> CommitAdminBatchAsync(
        [FromBody] AdminGpuPhotoBatchCommitRequestDto request)
    {
        string userId = User.FindFirstValue(ClaimTypes.Sid);
        if(string.IsNullOrEmpty(userId)) return Unauthorized();
        ...
        List<AdminGpuPhotoBatchCommitItemDto> items = request.Items;
        int gpuId = request.GpuId;
        int licenseId = request.LicenseId;

        _ = Task.Run(() => RunBatchJobAsync(jobId, userId, gpuId, licenseId, items, assetRootPath, scopeFactory));

        AdminGpuPhotoBatchJobStatusDto snapshot = SnapshotJob(batchJobs.GetForOwner(jobId, userId)!);
        return Accepted(snapshot);
    }

    /// <summary>
    ///     Poll the status of an in-flight admin GPU-photo batch-commit job. Owner-only.
    /// </summary>
    ...
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public ActionResult<AdminGpuPhotoBatchJobStatusDto> GetAdminBatchStatus(Guid jobId)
    {
        string userId = User.FindFirstValue(ClaimTypes.Sid);
        if(string.IsNullOrEmpty(userId)) return Unauthorized();
    ...
    static AdminGpuPhotoBatchJobStatusDto SnapshotJob(BatchUploadJobStore.BatchJob job)
    {
        lock(job.Lock)
        {
            return new AdminGpuPhotoBatchJobStatusDto
            {
                JobId            = job.JobId,
                State            = (int)job.State,
                Total            = job.Total,
                ...
                Results          = job.Results.Select(r => new AdminGpuPhotoBatchJobItemResultDto
                                       {
    ...
    /// <summary>
    ///     Sequential worker that promotes each pending image into a GpuPhoto row,
    ...
    /// </summary>
    async Task RunBatchJobAsync(Guid jobId, string userId, int gpuId, int licenseId,
                                List<AdminGpuPhotoBatchCommitItemDto> items, string assetRootPath,
                                IServiceScopeFactory scopeFactory)
    {
        ...
        {
            AdminGpuPhotoBatchCommitItemDto item = items[i];
            ...
            try
            {
                ...
                string originalsDir = Path.Combine(assetRootPath, "photos", "gpus", "originals");
                ...
                using(IServiceScope scope = scopeFactory.CreateScope())
                {
                    ...
                    await ctx.GpuPhotos.AddAsync(model);
                ...
                var photos = new Photos();
                photos.ConversionWorker(assetRootPath, photoId, finalPath, ext, false, "gpus");
