...
public class SoundSynthPhotosController(MarechaiContext context, IConfiguration configuration) : ControllerBase
{
    ...
    [HttpGet("/sound-synths/{soundSynthId:int}/photos")]
    [AllowAnonymous]
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public Task<SoundSynthPhotoDto> GetAsync(Guid id) => context.SoundSynthPhotos.Where(p => p.Id == id)
                                                         .Select(p => new SoundSynthPhotoDto
                                                          {
                                                              Aperture              = p.Aperture,
                                                              Author                = p.Author,
                                                              ...
                                                              LightSource = p.LightSource.HasValue
                                                                  ...
                                                                  : null,
                                                              SoundSynthCompanyName = p.SoundSynth.Company.Name,
                                                              SoundSynthId          = p.SoundSynthId,
                                                              SoundSynthName        = p.SoundSynth.Name,
    ...
    public async Task<ActionResult> UpdateAsync(Guid id, [FromBody] SoundSynthPhotoDto dto)
    {
        string userId = User.FindFirstValue(ClaimTypes.Sid);

        if(userId is null) return Unauthorized();
        SoundSynthPhoto model = await context.SoundSynthPhotos.FindAsync(id);
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    public async Task<ActionResult<Guid>> CreateAsync([FromBody] SoundSynthPhotoDto dto)
    {
        string userId = User.FindFirstValue(ClaimTypes.Sid);
        ...
        await context.SoundSynthPhotos.AddAsync(model);
        await context.SaveChangesWithUserAsync(userId);
                                                                    ...
                                                                    [FromForm] string   source)
    {
        ...
        Photos.EnsureCreated(_assetRootPath, false, "sound-synths");

        string originalsDir = Path.Combine(_assetRootPath, "photos", "sound-synths", "originals");
        ...
        _ = Task.Run(() =>
        {
            ...
            photos.ConversionWorker(_assetRootPath, model.Id, originalPath, sourceFormat, false, "sound-synths");
        });
        ...
        return Ok(new SoundSynthPhotoDto
        {
            Id                    = model.Id,
            ...
            SoundSynthId          = model.SoundSynthId,
    ...
    public async Task<ActionResult> DeleteAsync(Guid id)
    {
        ...
        context.SoundSynthPhotos.Remove(model);
        await context.SaveChangesWithUserAsync(userId);

        // Delete all generated files from disk
        string photosRoot = Path.Combine(_assetRootPath, "photos", "sound-synths");
    ...
    ///     sound synth before they submit (or cancel) the suggestion. Mirrors the per-batch
    ///     cap enforced by the dialog.
    /// </summary>
    const int PendingPhotosPerUserPerSoundSynthCap = 15;

    /// <summary>Allowed extensions for collaborator-uploaded sound synth photos (must match server JS validation).</summary>
    static readonly HashSet<string> _pendingAllowedExtensions = new(StringComparer.OrdinalIgnoreCase)
    {
        ".jpg", ".jpeg", ".png", ".webp"
    };

    /// <summary>Allowed content types for collaborator-uploaded sound synth photos (must match server JS validation).</summary>
    ...
    /// <summary>
    ///     Stage a single pending sound synth photo for a brand-new collaborative suggestion.
    ///     The uploader keeps each pending file on the server (sidecar tracks ownership +
    ///     parent <c>soundSynthId</c>) until they call <c>POST /suggestions</c> referencing
        ...
        [FromQuery] int soundSynthId)
    {
        ...
        await using(Stream stream = file.OpenReadStream())
        {
            ...
            // doesn't exist yet. ParentEntityId carries the soundSynthId so the per-uploader
    ...
    /// <summary>
    ///     Stream the binary contents of a pending sound synth photo. Used by the dialog
    ...
    public async Task<ActionResult> GetPendingAsync(Guid guid)
    {
        ...
        string path = await PendingImageStore.GetImagePathAsync(_assetRootPath, "sound-synths", guid);
