...
/// <summary>
///     Applies a per-language machine description suggestion. The suggestion's <c>Subkey</c>
...
/// </summary>
internal static class MachineDescriptionSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) description for the given machine / language. Returns
    ///     <c>null</c> when the machine itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the machine exists but has no description in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Machines.AsNoTracking().AnyAsync(c => c.Id == (int)entityId);
        if(!exists) return null;

        MachineDescription d =
            await context.MachineDescriptions.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.MachineId == (int)entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>MachineDescription</c> row. Renders
    ///     the markdown to HTML server-side via Markdig with advanced extensions, mirroring
    ///     <c>MachinesController.CreateOrUpdateDescriptionAsync</c>. Empty / whitespace-only
        ...
        HashSet<string> accepted)
    {
        ...
        bool exists = await context.Machines.AnyAsync(c => c.Id == (int)entityId);
        ...
        // Render to HTML server-side (mirrors MachinesController.CreateOrUpdateDescriptionAsync).
        ...
        MachineDescription current =
            await context.MachineDescriptions.FirstOrDefaultAsync(d => d.MachineId == (int)entityId &&
