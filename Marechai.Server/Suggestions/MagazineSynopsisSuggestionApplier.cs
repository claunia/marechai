...
/// <summary>
///     Applies a per-language magazine synopsis suggestion. The suggestion's <c>Subkey</c>
///     holds the ISO-639-3 language code; the <c>SuggestedValues</c> payload contains a single
///     <c>markdown</c> field with the proposed synopsis body.
/// </summary>
internal static class MagazineSynopsisSuggestionApplier
{
    ...
    /// <summary>
    ///     Languages allowed as a Subkey for synopsis suggestions. Mirrors the seeded set in
    ...
    /// <summary>
    ///     Read the current (markdown) synopsis for the given magazine / language. Returns
    ///     <c>null</c> when the magazine itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the magazine exists but has no synopsis in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Magazines.AsNoTracking().AnyAsync(m => m.Id == entityId);
        if(!exists) return null;

        MagazineSynopsis s =
            await context.MagazineSynopses.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.MagazineId == entityId &&
                                                   x.LanguageCode == languageCode);

        return new Dictionary<string, object>(StringComparer.Ordinal)
        {
            [FieldMarkdown] = s?.Text ?? string.Empty
        };
    }

    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>MagazineSynopsis</c> row. Empty /
    ///     whitespace-only markdown is rejected upstream by the controller — this applier never
    ///     deletes. Note that <c>MagazineSynopsis</c> stores only the raw markdown text (no separate
    ///     rendered HTML column); rendering is performed by the public View page on demand.
    /// </summary>
        ...
        string creditedUserId)
    {
        ...
        bool exists = await context.Magazines.AnyAsync(m => m.Id == entityId);
        ...
        MagazineSynopsis current =
            await context.MagazineSynopses.FirstOrDefaultAsync(s => s.MagazineId == entityId &&
