...
/// <summary>
///     Applier for collaborative Software video link suggestions
///     (<see cref="SuggestionEntityType.SoftwareVideo" />).
///     <para>
///         A single suggestion row carries one brand-new YouTube video link to be added to an
///         existing <see cref="Marechai.Database.Models.Software" />. The user types only a
...
///         the per-(user, software, video) Pending dedupe gates resubmission of the same
///         video while a previous suggestion is still pending.
///     </para>
///     <para>
///         Acceptance creates a new <see cref="SoftwareVideo" /> row with
///         <c>Provider = "YouTube"</c>. Only YouTube URLs are currently supported.
///     </para>
/// </summary>
public static class SoftwareVideoSuggestionApplier
{
    /// <summary>
    ...
    /// <summary>
    ///     Validate the field-name keys carried by a SoftwareVideo suggestion. Recognises
    ...
    /// <summary>
    ///     Returns an empty (but non-null) dictionary when the parent Software exists, else
    ...
    ///     the parent Software exists, the URL parses to a YouTube video ID, the (software,
    ///     YouTube, videoId) triple is not already present in the <c>SoftwareVideos</c>
        ...
        IHttpClientFactory httpFactory, CancellationToken ct = default)
    {
        ...
        if(!entityId.HasValue || entityId.Value <= 0)
            return (false, "Software video suggestions must reference an existing software via entity_id.", null,
                    null);

        ulong softwareId = (ulong)entityId.Value;
        bool softwareExists = await context.Softwares.AsNoTracking().AnyAsync(s => s.Id == softwareId);
        if(!softwareExists) return (false, $"Software #{softwareId} not found.", null, null);

        // ── URL (mandatory) ────────────────────────────────────────────────────
        if(!suggested.TryGetValue(FieldVideoUrl, out object urlRaw) || urlRaw is null)
            return (false, "A software video suggestion must include a 'video_url' field.", null, null);
        ...
        // ── Duplicate check against existing SoftwareVideos ─────────────────────
        bool duplicate = await context.SoftwareVideos.AsNoTracking()
                                      .AnyAsync(v => v.SoftwareId == softwareId   &&
                                                     v.Provider   == ProviderName &&
                                                     v.VideoId    == videoId);

        if(duplicate)
            return (false, "This video is already linked to this software.", null, null);
    ...
    ///     create a new <see cref="SoftwareVideo" /> row with <c>Provider = "YouTube"</c>,
        ...
        long entityId, Dictionary<string, object> suggested, HashSet<string> accepted, string creditedUserId)
    {
        ...
        bool duplicate = await context.SoftwareVideos.AnyAsync(v => v.SoftwareId == softwareId   &&
                                                                    v.Provider   == ProviderName &&
                                                                    v.VideoId    == videoId);
        ...
        await context.SoftwareVideos.AddAsync(video);
