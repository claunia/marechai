...
/// <summary>
///     Applies a per-language SoundSynth description suggestion. The suggestion's <c>Subkey</c>
...
/// </summary>
internal static class SoundSynthDescriptionSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) description for the given SoundSynth / language. Returns
    ///     <c>null</c> when the SoundSynth itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the SoundSynth exists but has no description in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.SoundSynths.AsNoTracking().AnyAsync(s => s.Id == (int)entityId);
        if(!exists) return null;

        SoundSynthDescription d =
            await context.SoundSynthDescriptions.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.SoundSynthId == (int)entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>SoundSynthDescription</c> row. Renders
    ///     the markdown to HTML server-side via Markdig with advanced extensions, mirroring
    ///     <c>SoundSynthsController.CreateOrUpdateDescriptionAsync</c>. Empty / whitespace-only
        ...
        HashSet<string> accepted)
    {
        ...
        bool exists = await context.SoundSynths.AnyAsync(s => s.Id == (int)entityId);
        ...
        // Render to HTML server-side (mirrors SoundSynthsController.CreateOrUpdateDescriptionAsync).
        ...
        SoundSynthDescription current =
            await context.SoundSynthDescriptions.FirstOrDefaultAsync(d => d.SoundSynthId == (int)entityId &&
