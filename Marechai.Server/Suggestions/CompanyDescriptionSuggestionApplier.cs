...
/// <summary>
///     Applies a per-language company description suggestion. The suggestion's <c>Subkey</c>
...
/// </summary>
internal static class CompanyDescriptionSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) description for the given company / language. Returns
    ///     <c>null</c> when the company itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the company exists but has no description in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Companies.AsNoTracking().AnyAsync(c => c.Id == (int)entityId);
        if(!exists) return null;

        CompanyDescription d =
            await context.CompanyDescriptions.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.CompanyId == (int)entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>CompanyDescription</c> row. Renders
    ///     the markdown to HTML server-side via Markdig with advanced extensions, mirroring
    ///     <c>CompaniesController.CreateOrUpdateDescriptionAsync</c>. Empty / whitespace-only
        ...
        HashSet<string> accepted)
    {
        ...
        bool exists = await context.Companies.AnyAsync(c => c.Id == (int)entityId);
        ...
        // Render to HTML server-side (mirrors CompaniesController.CreateOrUpdateDescriptionAsync).
        ...
        CompanyDescription current =
            await context.CompanyDescriptions.FirstOrDefaultAsync(d => d.CompanyId == (int)entityId &&
