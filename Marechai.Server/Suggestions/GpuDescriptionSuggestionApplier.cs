...
/// <summary>
///     Applies a per-language GPU description suggestion. The suggestion's <c>Subkey</c>
...
/// </summary>
internal static class GpuDescriptionSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) description for the given GPU / language. Returns
    ///     <c>null</c> when the GPU itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the GPU exists but has no description in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Gpus.AsNoTracking().AnyAsync(g => g.Id == (int)entityId);
        if(!exists) return null;

        GpuDescription d =
            await context.GpuDescriptions.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.GpuId == (int)entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>GpuDescription</c> row. Renders
    ///     the markdown to HTML server-side via Markdig with advanced extensions, mirroring
    ///     <c>GpusController.CreateOrUpdateDescriptionAsync</c>. Empty / whitespace-only
        ...
        string creditedUserId)
    {
        ...
        bool exists = await context.Gpus.AnyAsync(g => g.Id == (int)entityId);
        ...
        // Render to HTML server-side (mirrors GpusController.CreateOrUpdateDescriptionAsync).
        ...
        GpuDescription current =
            await context.GpuDescriptions.FirstOrDefaultAsync(d => d.GpuId == (int)entityId &&
