...
/// <summary>
///     Applies a per-language book synopsis suggestion. The suggestion's <c>Subkey</c>
...
/// </summary>
internal static class BookSynopsisSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) synopsis for the given book / language. Returns
    ///     <c>null</c> when the book itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the book exists but has no synopsis in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Books.AsNoTracking().AnyAsync(b => b.Id == entityId);
        if(!exists) return null;

        BookSynopsis d =
            await context.BookSynopses.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.BookId == entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>BookSynopsis</c> row. Empty /
    ///     whitespace-only markdown is rejected upstream by the controller — this applier never
    ///     deletes. Note that <c>BookSynopsis</c> stores only the raw markdown text (no separate
        ...
        string creditedUserId)
    {
        ...
        bool exists = await context.Books.AnyAsync(b => b.Id == entityId);
        ...
        BookSynopsis current =
            await context.BookSynopses.FirstOrDefaultAsync(d => d.BookId == entityId &&
