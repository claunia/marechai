...
/// <summary>
///     Applies a per-language Software description suggestion. The suggestion's
///     <c>Subkey</c> holds the ISO-639-3 language code; the <c>SuggestedValues</c> payload
///     contains a single <c>markdown</c> field with the proposed description body.
/// </summary>
internal static class SoftwareDescriptionSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) description for the given Software / language. Returns
    ///     <c>null</c> when the Software itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the Software exists but has no description in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Softwares.AsNoTracking().AnyAsync(s => s.Id == (ulong)entityId);
        if(!exists) return null;

        SoftwareDescription d =
            await context.SoftwareDescriptions.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.SoftwareId == (ulong)entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>SoftwareDescription</c> row. Renders
    ///     the markdown to HTML server-side via Markdig with advanced extensions, mirroring
    ///     <c>SoftwareController.CreateOrUpdateDescriptionAsync</c>. Empty / whitespace-only
        ...
        string creditedUserId)
    {
        ...
        bool exists = await context.Softwares.AnyAsync(s => s.Id == (ulong)entityId);
        ...
        // Render to HTML server-side (mirrors SoftwareController.CreateOrUpdateDescriptionAsync).
        ...
        SoftwareDescription current =
            await context.SoftwareDescriptions.FirstOrDefaultAsync(d => d.SoftwareId == (ulong)entityId &&
