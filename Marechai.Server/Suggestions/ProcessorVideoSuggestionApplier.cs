...
/// <summary>
///     Applier for collaborative Processor video link suggestions
///     (<see cref="SuggestionEntityType.ProcessorVideo" />).
///     <para>
///         A single suggestion row carries one brand-new YouTube video link to be added to an
///         existing <see cref="Marechai.Database.Models.Processor" />. The user types only a
...
///         the per-(user, processor, video) Pending dedupe gates resubmission of the same
///         video while a previous suggestion is still pending.
///     </para>
///     <para>
///         Acceptance creates a new <see cref="ProcessorVideo" /> row with
///         <c>Provider = "YouTube"</c>. Only YouTube URLs are currently supported.
///     </para>
/// </summary>
public static class ProcessorVideoSuggestionApplier
{
    /// <summary>
    ...
    /// <summary>
    ///     Validate the field-name keys carried by a ProcessorVideo suggestion. Recognises
    ...
    /// <summary>
    ///     Returns an empty (but non-null) dictionary when the parent Processor exists, else
    ...
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context, long entityId)
    {
        ...
        bool exists     = await context.Processors.AsNoTracking().AnyAsync(p => p.Id == processorId);
    ...
    ///     the parent Processor exists, the URL parses to a YouTube video ID, the (processor,
    ///     YouTube, videoId) triple is not already present in the <c>ProcessorVideos</c>
        ...
        IHttpClientFactory httpFactory, CancellationToken ct = default)
    {
        ...
        if(!entityId.HasValue || entityId.Value <= 0)
            return (false, "Processor video suggestions must reference an existing processor via entity_id.", null,
                    null);

        int processorId = (int)entityId.Value;
        bool processorExists = await context.Processors.AsNoTracking().AnyAsync(p => p.Id == processorId);
        if(!processorExists) return (false, $"Processor #{processorId} not found.", null, null);

        // ── URL (mandatory) ────────────────────────────────────────────────────
        if(!suggested.TryGetValue(FieldVideoUrl, out object urlRaw) || urlRaw is null)
            return (false, "A processor video suggestion must include a 'video_url' field.", null, null);
        ...
        // ── Duplicate check against existing ProcessorVideos ───────────────────
        bool duplicate = await context.ProcessorVideos.AsNoTracking()
                                      .AnyAsync(v => v.ProcessorId == processorId   &&
                                                     v.Provider    == ProviderName  &&
                                                     v.VideoId     == videoId);

        if(duplicate)
            return (false, "This video is already linked to this processor.", null, null);
    ...
    ///     create a new <see cref="ProcessorVideo" /> row with <c>Provider = "YouTube"</c>,
    ...
    ///     flag indicating the parent Processor was missing (so the controller can mark the
    ///     suggestion stale).
    /// </summary>
        ...
        long entityId, Dictionary<string, object> suggested, HashSet<string> accepted, string creditedUserId)
    {
        ...
        bool duplicate = await context.ProcessorVideos.AnyAsync(v => v.ProcessorId == processorId   &&
                                                                      v.Provider    == ProviderName  &&
                                                                      v.VideoId     == videoId);
        ...
        await context.ProcessorVideos.AddAsync(video);
