...
/// <summary>
///     Applies a per-language document synopsis suggestion. The suggestion's <c>Subkey</c>
...
/// </summary>
internal static class DocumentSynopsisSuggestionApplier
{
    ...
    /// <summary>
    ///     Read the current (markdown) synopsis for the given document / language. Returns
    ///     <c>null</c> when the document itself does not exist (sentinel for "entity missing");
    ///     returns <c>{ "markdown": "" }</c> when the document exists but has no synopsis in
    ///     this language yet.
    /// </summary>
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context,
                                                                               ...
                                                                               string languageCode)
    {
        bool exists = await context.Documents.AsNoTracking().AnyAsync(d => d.Id == entityId);
        if(!exists) return null;

        DocumentSynopsis d =
            await context.DocumentSynopses.AsNoTracking()
                         .FirstOrDefaultAsync(x => x.DocumentId == entityId &&
    ...
    /// <summary>
    ///     Apply the accepted markdown to the per-language <c>DocumentSynopsis</c> row. Empty /
    ///     whitespace-only markdown is rejected upstream by the controller — this applier never
    ///     deletes. Note that <c>DocumentSynopsis</c> stores only the raw markdown text (no separate
        ...
        string creditedUserId)
    {
        ...
        bool exists = await context.Documents.AnyAsync(d => d.Id == entityId);
        ...
        DocumentSynopsis current =
            await context.DocumentSynopses.FirstOrDefaultAsync(d => d.DocumentId == entityId &&
