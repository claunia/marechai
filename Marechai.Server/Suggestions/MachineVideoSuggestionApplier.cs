...
/// <summary>
///     Applier for collaborative Machine video link suggestions
///     (<see cref="SuggestionEntityType.MachineVideo" />).
///     <para>
///         A single suggestion row carries one brand-new YouTube video link to be added to an
///         existing <see cref="Marechai.Database.Models.Machine" />. The user types only a
...
///         the per-(user, machine, video) Pending dedupe gates resubmission of the same
///         video while a previous suggestion is still pending.
///     </para>
///     <para>
///         Acceptance creates a new <see cref="MachineVideo" /> row with
///         <c>Provider = "YouTube"</c>. Only YouTube URLs are currently supported.
///     </para>
/// </summary>
public static class MachineVideoSuggestionApplier
{
    /// <summary>
    ...
    /// <summary>
    ///     Validate the field-name keys carried by a MachineVideo suggestion. Recognises
    ...
    /// <summary>
    ///     Returns an empty (but non-null) dictionary when the parent Machine exists, else
    ...
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context, long entityId)
    {
        ...
        bool exists     = await context.Machines.AsNoTracking().AnyAsync(p => p.Id == machineId);
    ...
    ///     the parent Machine exists, the URL parses to a YouTube video ID, the (machine,
    ///     YouTube, videoId) triple is not already present in the <c>MachineVideos</c>
        ...
        IHttpClientFactory httpFactory, CancellationToken ct = default)
    {
        ...
        if(!entityId.HasValue || entityId.Value <= 0)
            return (false, "Machine video suggestions must reference an existing machine via entity_id.", null,
                    null);

        int machineId = (int)entityId.Value;
        bool machineExists = await context.Machines.AsNoTracking().AnyAsync(p => p.Id == machineId);
        if(!machineExists) return (false, $"Machine #{machineId} not found.", null, null);

        // ── URL (mandatory) ────────────────────────────────────────────────────
        if(!suggested.TryGetValue(FieldVideoUrl, out object urlRaw) || urlRaw is null)
            return (false, "A machine video suggestion must include a 'video_url' field.", null, null);
        ...
        // ── Duplicate check against existing MachineVideos ───────────────────
        bool duplicate = await context.MachineVideos.AsNoTracking()
                                      .AnyAsync(v => v.MachineId == machineId   &&
                                                     v.Provider    == ProviderName  &&
                                                     v.VideoId     == videoId);

        if(duplicate)
            return (false, "This video is already linked to this machine.", null, null);
    ...
    ///     create a new <see cref="MachineVideo" /> row with <c>Provider = "YouTube"</c>,
    ...
    ///     flag indicating the parent Machine was missing (so the controller can mark the
    ///     suggestion stale).
    /// </summary>
        ...
        long entityId, Dictionary<string, object> suggested, HashSet<string> accepted, string creditedUserId)
    {
        ...
        int machineId        = (int)entityId;
        bool machineExists   = await context.Machines.AnyAsync(p => p.Id == machineId);
        ...
        bool duplicate = await context.MachineVideos.AnyAsync(v => v.MachineId == machineId   &&
                                                                      v.Provider    == ProviderName  &&
                                                                      v.VideoId     == videoId);
        ...
        await context.MachineVideos.AddAsync(video);
