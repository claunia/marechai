...
/// <summary>
///     Applier for collaborative GPU video link suggestions
///     (<see cref="SuggestionEntityType.GpuVideo" />).
///     <para>
///         A single suggestion row carries one brand-new YouTube video link to be added to an
///         existing <see cref="Marechai.Database.Models.Gpu" />. The user types only a URL
///         (or 11-character video ID); the canonical title is fetched server-side from
...
///         the per-(user, gpu, video) Pending dedupe gates resubmission of the same video
///         while a previous suggestion is still pending.
///     </para>
///     <para>
///         Acceptance creates a new <see cref="GpuVideo" /> row with
///         <c>Provider = "YouTube"</c>. Only YouTube URLs are currently supported.
///     </para>
/// </summary>
public static class GpuVideoSuggestionApplier
{
    /// <summary>
    ...
    /// <summary>
    ///     Validate the field-name keys carried by a GpuVideo suggestion. Recognises only the
    ///     two literal scalars (<see cref="FieldVideoUrl" />, <see cref="FieldTitle" />); no
    ///     dynamic keys. Used both by the controller's blanket field-name allow-list check
    ...
    /// <summary>
    ///     Returns an empty (but non-null) dictionary when the parent GPU exists, else
    ...
    public static async Task<Dictionary<string, object>> GetCurrentValuesAsync(MarechaiContext context, long entityId)
    {
        ...
        bool exists = await context.Gpus.AsNoTracking().AnyAsync(g => g.Id == gpuId);
    ...
    ///     the parent GPU exists, the URL parses to a YouTube video ID, the (gpu, YouTube,
    ///     videoId) triple is not already present in the <c>GpuVideos</c> table, and fetches
    ///     the canonical title from YouTube's oEmbed endpoint. Returns
        ...
        IHttpClientFactory httpFactory, CancellationToken ct = default)
    {
        ...
        if(!entityId.HasValue || entityId.Value <= 0)
            return (false, "GPU video suggestions must reference an existing GPU via entity_id.", null, null);

        int gpuId = (int)entityId.Value;
        bool gpuExists = await context.Gpus.AsNoTracking().AnyAsync(g => g.Id == gpuId);
        if(!gpuExists) return (false, $"GPU #{gpuId} not found.", null, null);

        // ── URL (mandatory) ────────────────────────────────────────────────────
        if(!suggested.TryGetValue(FieldVideoUrl, out object urlRaw) || urlRaw is null)
            return (false, "A GPU video suggestion must include a 'video_url' field.", null, null);
        ...
        // ── Duplicate check against existing GpuVideos ─────────────────────────
        bool duplicate = await context.GpuVideos.AsNoTracking()
                                      .AnyAsync(v => v.GpuId    == gpuId         &&
                                                     v.Provider == ProviderName  &&
                                                     v.VideoId  == videoId);

        if(duplicate)
            return (false, "This video is already linked to this GPU.", null, null);
    ...
    ///     create a new <see cref="GpuVideo" /> row with <c>Provider = "YouTube"</c>,
    ...
    ///     flag indicating the parent GPU was missing (so the controller can mark the
    ///     suggestion stale).
    /// </summary>
        ...
        long entityId, Dictionary<string, object> suggested, HashSet<string> accepted, string creditedUserId)
    {
        ...
        bool duplicate = await context.GpuVideos.AnyAsync(v => v.GpuId    == gpuId        &&
                                                                v.Provider == ProviderName &&
                                                                v.VideoId  == videoId);
        ...
        await context.GpuVideos.AddAsync(video);
