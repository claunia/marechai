...
public sealed class MagazineIssueCoverCache
{
    readonly IConfiguration _configuration;
    StorageFolder           _coversFolder;
    StorageFolder           _thumbnailsFolder;

    public MagazineIssueCoverCache(IConfiguration configuration)
    {
    ...
    async Task EnsureFolderExistAsync()
    {
        ...
        _thumbnailsFolder = await localFolder.CreateFolderAsync("magazine_issue_cover_thumbnails",
                                                                  CreationCollisionOption.OpenIfExists);

        _coversFolder = await localFolder.CreateFolderAsync("magazine_issue_covers",
                                                              CreationCollisionOption.OpenIfExists);
    }
    ...
    async Task CacheThumbnailAsync(Guid coverId)
    {
        ...
        string url      = baseUrl + $"/assets/photos/magazine-issue-covers/thumbs/webp/4k/{filename}";
    ...
    async Task CacheCoverAsync(Guid coverId)
    {
        var    filename = $"{coverId}.webp";
        string baseUrl  = _configuration.GetSection("ApiClient:Url").Value;
        string url      = baseUrl + $"/assets/photos/magazine-issue-covers/webp/4k/{filename}";
