...
public sealed class SoundSynthPhotoCache
{
    readonly IConfiguration _configuration;
    StorageFolder           _photosFolder;
    StorageFolder           _thumbnailsFolder;

    public SoundSynthPhotoCache(IConfiguration configuration)
    {
    ...
    async Task EnsureFolderExistAsync()
    {
        ...
        _thumbnailsFolder =
            await localFolder.CreateFolderAsync("sound_synth_thumbnails", CreationCollisionOption.OpenIfExists);

        _photosFolder = await localFolder.CreateFolderAsync("sound_synth_photos", CreationCollisionOption.OpenIfExists);
    }
    ...
    async Task CacheThumbnailAsync(Guid photoId)
    {
        ...
        string                    flagUrl    = baseUrl + $"/assets/photos/sound-synths/thumbs/webp/4k/{filename}";
    ...
    async Task CachePhotoAsync(Guid photoId)
    {
        var                       filename   = $"{photoId}.webp";
        string                    baseUrl    = _configuration.GetSection("ApiClient:Url").Value;
        string                    flagUrl    = baseUrl + $"/assets/photos/sound-synths/webp/4k/{filename}";
        using var                 httpClient = new HttpClient();
        using HttpResponseMessage response   = await httpClient.GetAsync(flagUrl);
        response.EnsureSuccessStatusCode();
