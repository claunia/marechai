...
public sealed class SoftwareCoverCache
{
    readonly IConfiguration _configuration;
    StorageFolder           _coversFolder;
    StorageFolder           _thumbnailsFolder;

    public SoftwareCoverCache(IConfiguration configuration)
    {
    ...
    async Task EnsureFolderExistAsync()
    {
        ...
        _thumbnailsFolder =
            await localFolder.CreateFolderAsync("software_cover_thumbnails", CreationCollisionOption.OpenIfExists);

        _coversFolder =
            await localFolder.CreateFolderAsync("software_covers", CreationCollisionOption.OpenIfExists);
    }
    ...
    async Task CacheCoverAsync(Guid coverId)
    {
        ...
        StorageFile file =
            await _coversFolder.CreateFileAsync(filename, CreationCollisionOption.ReplaceExisting);
