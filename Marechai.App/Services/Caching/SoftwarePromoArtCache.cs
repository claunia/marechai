...
public sealed class SoftwarePromoArtCache
{
    readonly IConfiguration _configuration;
    StorageFolder           _promoArtFolder;
    StorageFolder           _thumbnailsFolder;

    public SoftwarePromoArtCache(IConfiguration configuration)
    {
    ...
    async Task EnsureFolderExistAsync()
    {
        ...
        _thumbnailsFolder =
            await localFolder.CreateFolderAsync("software_promo_art_thumbnails", CreationCollisionOption.OpenIfExists);

        _promoArtFolder =
            await localFolder.CreateFolderAsync("software_promo_art", CreationCollisionOption.OpenIfExists);
    }
    ...
    async Task CacheThumbnailAsync(Guid promoArtId)
    {
        ...
        string url      = baseUrl + $"/assets/photos/software-promo-art/thumbs/webp/4k/{filename}";
    ...
    async Task CachePromoArtAsync(Guid promoArtId)
    {
        var    filename = $"{promoArtId}.webp";
        string baseUrl  = _configuration.GetSection("ApiClient:Url").Value;
        string url      = baseUrl + $"/assets/photos/software-promo-art/webp/4k/{filename}";
