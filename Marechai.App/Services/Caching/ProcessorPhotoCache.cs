...
public sealed class ProcessorPhotoCache
{
    readonly IConfiguration _configuration;
    StorageFolder           _photosFolder;
    StorageFolder           _thumbnailsFolder;

    public ProcessorPhotoCache(IConfiguration configuration)
    {
    ...
    async Task EnsureFolderExistAsync()
    {
        ...
        _thumbnailsFolder =
            await localFolder.CreateFolderAsync("processor_thumbnails", CreationCollisionOption.OpenIfExists);

        _photosFolder = await localFolder.CreateFolderAsync("processor_photos", CreationCollisionOption.OpenIfExists);
    }
    ...
    async Task CacheThumbnailAsync(Guid photoId)
    {
        ...
        string                    flagUrl    = baseUrl + $"/assets/photos/processors/thumbs/webp/4k/{filename}";
    ...
    async Task CachePhotoAsync(Guid photoId)
    {
        var                       filename   = $"{photoId}.webp";
        string                    baseUrl    = _configuration.GetSection("ApiClient:Url").Value;
        string                    flagUrl    = baseUrl + $"/assets/photos/processors/webp/4k/{filename}";
        using var                 httpClient = new HttpClient();
        using HttpResponseMessage response   = await httpClient.GetAsync(flagUrl);
        response.EnsureSuccessStatusCode();
