...
public partial class GpuPhotoDetailViewModel : ObservableObject, IRegionAware
{
    private readonly GpusService                    _gpusService;
    private readonly ImageSourceFactory              _imageSourceFactory;
    private readonly IStringLocalizer                _localizer;
    private readonly ILogger<GpuPhotoDetailViewModel> _logger;
    private readonly IRegionManager                  _regionManager;
    private readonly GpuPhotoCache                   _photoCache;
    ...
    [ObservableProperty]
    private string _photoGpuCompanyName = string.Empty;

    [ObservableProperty]
    private string _photoGpuName = string.Empty;
    ...
    public GpuPhotoDetailViewModel(ILogger<GpuPhotoDetailViewModel> logger, IRegionManager regionManager,
                                   GpusService                      gpusService,    GpuPhotoCache      photoCache,
    ...
    [RelayCommand]
    public Task NavigateToGpu()
    {
        if(_gpuId <= 0) return Task.CompletedTask;

        var parameters = new NavigationParameters
        {
            { NavParamKeys.GpuId, _gpuId },
            { NavParamKeys.NavigationSource, nameof(GpuPhotoDetailViewModel) }
        };
    ...
    public async Task LoadPhoto(Guid photoId)
    {
        try
        {
            ...
            _logger.LogInformation("Loading GPU photo details for {PhotoId}", photoId);

            // Fetch photo details from API
            GpuPhotoDto? photo = await _gpusService.GetGpuPhotoDetailsAsync(photoId);
            ...
            PhotoGpuCompanyName     = photo.GpuCompanyName     ?? string.Empty;
            PhotoGpuName            = photo.GpuName            ?? string.Empty;
        ...
        {
            _logger.LogError(ex, "Error loading GPU photo details for {PhotoId}", photoId);
            ErrorOccurred = true;
    ...
    private async Task LoadPhotoImageAsync(Guid photoId)
    {
        ...
        {
            _logger.LogError(ex, "Error loading GPU photo image {PhotoId}", photoId);
            ErrorOccurred = true;
