...
/// <summary>
/// Consolidated payload for the public /processor/{Id} view page. Replaces 5 sequential
...
public class ProcessorFullDto
{
    ...
    // inner DTO un-populated. See pattern in BookFullDto.Book and GpuFullDto.Gpu.
    [JsonPropertyName("processor")]
    [Required]
    public ProcessorDto Processor { get; set; }

    /// <summary>
    /// Asset GUID of the company logo to display next to the processor header. Picked from
    /// the company's logos with the introduced-year preference applied server-side (logos
    /// with <c>Year &gt;= introducedYear</c> sort first, then ascending by year). Null when
    /// the processor has no company or the company has no logos.
    /// </summary>
    ...
    /// server-side via a single ordered query). Null at runtime when the processor has no
    ...
    [Required]
    public List<ProcessorVideoDto> Videos { get; set; } = new();
}
