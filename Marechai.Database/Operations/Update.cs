...
public partial class Operations
{
    ...
    {
        bool dbV2             = !core.TableExists("marechai_db");
        var  currentDbVersion = 2;

        if(!dbV2)
        {
            IDbCommand     dbCmd       = connection.CreateCommand();
            IDbDataAdapter dataAdapter = core.GetNewDataAdapter();
            ...
            {
                var newId = int.Parse(dataRow["version"].ToString());
    ...
    void UpdateDatabaseToV3()
    {
        ...
        dbCmd.CommandText = @"ALTER TABLE `computers` CHANGE COLUMN `spu` `sound_synth` int(11) NOT NULL DEFAULT '0'";

        dbCmd.ExecuteNonQuery();
        Console.WriteLine("Renaming column `computers.mpu` to `music_synth.name`");
        dbCmd = connection.CreateCommand();

        dbCmd.CommandText = @"ALTER TABLE `computers` CHANGE COLUMN `mpu` `music_synth` int(11) NOT NULL DEFAULT '0'";
        ...
        dbCmd.CommandText = @"ALTER TABLE `consoles` CHANGE COLUMN `spu` `sound_synth` int(11) NOT NULL DEFAULT '0'";

        dbCmd.ExecuteNonQuery();
        Console.WriteLine("Renaming column `consoles.mpu` to `consoles.music_synth`");
        dbCmd = connection.CreateCommand();

        dbCmd.CommandText = @"ALTER TABLE `consoles` CHANGE COLUMN `mpu` `music_synth` int(11) NOT NULL DEFAULT '0'";
        ...
        {
            dbCmd                     = connection.CreateCommand();
            dataAdapter               = core.GetNewDataAdapter();
            ...
            {
                Console.WriteLine("Converting console company `{0}` to company `{1}`",
                                  consoleCompany.Value,
                                  dataSet.Tables[0].Rows[0]["name"]);

                conversionEquivalents.Add(consoleCompany.Key, int.Parse(dataSet.Tables[0].Rows[0]["id"].ToString()));
            }
            else
            {
        ...
        foreach(string file in
                Directory.GetFiles("wwwroot/assets/logos/computers/", "*", SearchOption.TopDirectoryOnly))
        {
        ...
        foreach(string file in Directory.GetFiles("wwwroot/assets/logos/consoles/", "*", SearchOption.TopDirectoryOnly))
        {
    ...
    void UpdateDatabaseToV12()
    {
        ...
        dbCmd.CommandText = $"INSERT INTO `gpus` (`id`, `name`) VALUES ({DbNone}, 'DB_NONE');\n" +
                            $"INSERT INTO `gpus` (`id`, `name`) VALUES ({DbSoftware}, 'DB_FRAMEBUFFER');";
    ...
    void UpdateDatabaseToV13()
    {
        ...
        foreach(KeyValuePair<int, string> musicSynth in musicSynths)
        {
            ...
            if(dataSet.Tables[0].Rows.Count == 1)
            {
                Console.WriteLine("Converting music synth `{0}` to sound synth `{1}`",
                                  musicSynth.Value,
                                  dataSet.Tables[0].Rows[0]["name"]);
    ...
    void UpdateDatabaseToV14()
    {
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            param4.Value = dataRow["cpu1"] == DBNull.Value ? null : (int)dataRow["cpu1"];

            param5.Value = dataRow["mhz1"] == DBNull.Value ? null : float.Parse(dataRow["mhz1"].ToString());

            param6.Value = dataRow["cpu2"] == DBNull.Value ? null : (int)dataRow["cpu2"];

            param7.Value = dataRow["mhz2"] == DBNull.Value ? null : float.Parse(dataRow["mhz2"].ToString());

            param8.Value  = (int)dataRow["ram"];
            param9.Value  = (int)dataRow["rom"];
            param10.Value = dataRow["gpu"] == DBNull.Value ? null : (int)dataRow["gpu"];
    ...
    void UpdateDatabaseToV15()
    {
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            if(dataRow["cpu1"] != DBNull.Value)
            {
                ...
                param3.Value = dataRow["mhz1"] == DBNull.Value ? null : float.Parse(dataRow["mhz1"].ToString());
            ...
            if(dataRow["cpu2"] != DBNull.Value)
            {
                ...
                param3.Value = dataRow["mhz2"] == DBNull.Value ? null : float.Parse(dataRow["mhz2"].ToString());
    ...
    void UpdateDatabaseToV17()
    {
        ...
        trans             = connection.BeginTransaction();
        dbCmd.Transaction = trans;
        dbCmd.CommandText = $"INSERT INTO `sound_synths` (`id`, `name`) VALUES ({DbSoftware}, 'DB_SOFTWARE');";
        dbCmd.ExecuteNonQuery();
        trans.Commit();
        dbCmd.Dispose();

        Console.WriteLine("Changing sound and music synths in machine from 27 to {0}", DbSoftware);
        dbCmd             = connection.CreateCommand();
        trans             = connection.BeginTransaction();
        dbCmd.Transaction = trans;

        dbCmd.CommandText = $"UPDATE `machines` SET sound_synth = {DbSoftware} WHERE sound_synth = 27;\n" +
                            $"UPDATE `machines` SET music_synth = {DbSoftware} WHERE music_synth = 27;";
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            const string sql = "INSERT INTO sound_by_machine (machine, sound_synth) VALUES (@machine, @sound_synth)";
    ...
    void UpdateDatabaseToV18()
    {
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            if(dataRow["ram"] != DBNull.Value && (int)dataRow["ram"] > 0)
            {
                ...
                Console.WriteLine("Adding {0} bytes of {1} memory to machine {2}",
                                  (int)param3.Value,
                                  MemoryUsage.Work,
                                  (int)param1.Value);
    ...
    void UpdateDatabaseToV19()
    {
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            dbCmd = connection.CreateCommand();
            IDbDataAdapter dataAdapter2 = core.GetNewDataAdapter();
            ...
            var gpuId = (int)dataSet2.Tables[0].Rows[0]["gpu"];
            ...
            dbCmd        = connection.CreateCommand();
            dataAdapter2 = core.GetNewDataAdapter();
            ...
            if(dataSet2.Tables[0].Rows.Count == 0)
            {
                ...
                resId = (int)core.LastInsertRowId;
            }
            else
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
                    ...
                    })
            {
                ...
                Console.WriteLine("Adding storage type {0} with interface {1} to machine {2}",
                                  (StorageType)param2.Value,
                                  (StorageInterface)param3.Value,
                                  (int)param1.Value);
    ...
    void UpdateDatabaseToV21()
    {
        ...
        foreach(DataRow dataRow in dataSet.Tables[0].Rows)
        {
            ...
            var sql = $"UPDATE `machines` SET introduced = @introduced WHERE id = {(int)dataRow["id"]}";
            ...
            Console.WriteLine("Adding introduction date {0} to machine {1}", (int)dataRow["year"], (int)dataRow["id"]);
    ...
    void UpdateDatabaseToV23()
    {
        ...
        dbCmd.CommandText = "ALTER TABLE gpus_by_machine       ADD id BIGINT NOT NULL PRIMARY KEY AUTO_INCREMENT;\n" +
